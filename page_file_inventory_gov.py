...
def render(engine, dialect: str):
    ...
    NAV_ITEMS = [
        ...
        ("shapefiles","🗺️", "Shapefiles", "Scan, classify and load spatial files to PPDM"),
...
def _tab_header_export(engine, dialect):
    """
    Export stored file headers as CSV/Excel for manager review and PPDM update.
    Supports staged upload → review → apply to PPDM.
    """
    import pandas as pd
    import io
    from sqlalchemy import text
    ...
    st.markdown("#### 📄 File Header Export & PPDM Update")
    st.caption(
        "Headers are captured automatically when files are cataloged. "
        "Export to CSV/Excel, edit, then upload to stage changes for PPDM."
    )
    ...
    with tab_well:
                ...
                if df.empty:
                    ...
                    if export_fmt == "CSV":
                        ...
                        with pd.ExcelWriter(buf, engine="openpyxl") as writer:
                            ...
                            tmpl.to_excel(writer, index=False, sheet_name="PPDM Update Template")
                        buf.seek(0)
                        st.download_button(
        ...
        st.divider()
        st.markdown("**Upload edited headers → stage for PPDM update**")
        st.caption("Upload the edited 'PPDM Update Template' sheet. "
            ...
            try:
                if uploaded.name.endswith(".csv"):
                    ...
                    df_up = pd.read_excel(uploaded,
                                          sheet_name="PPDM Update Template")
                ...
                try:
                    ...
                    for row in rows:
                        try:
                            ...
                            errors += 1
                    if errors == 0:
                        st.success(f"✅ {applied:,} well records updated in PPDM.")
                    else:
...
def _tab_office_extract(engine, dialect, user):
    ...
    st.caption(
        ...
        "well headers and more — then loads to PPDM."
    )
