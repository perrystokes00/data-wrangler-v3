...
SURVEYS = [
    {
        ...
        "surf_lat":  31.8234, "surf_lon": -102.4521,
    ...
    },
    {
        ...
        "surf_lat":  35.5, "surf_lon": -98.5,
        "operator":  "Continental Resources",
        "field":     "Anadarko Basin",
        "state":     "OK",
        "survey_type": "MWD",
...
def _norm_uwi(raw: str) -> str:
    """Convert API/UWI to DB format: US + 14 digits."""
    ...
    with engine.begin() as con:
        for s in surveys:
                ...
                try:
                    con.execute(text("""
                        INSERT INTO dataview.dv_well (
                            ...
                            province_state, onshore_offshore_ind,
                            active_ind, row_created_by, row_created_date, source
                        ) VALUES (
                            :u, :wn, 'HORIZONTAL', 'COMPLETED',
                            :lat, :lon,
                            :state, 'ONSHORE',
                            'Y', 'LOADER', GETDATE(), 'DATAVIEW'
                        )
                    """), {
                        "u":     uwi,
                        "wn":    s["well_name"],
                    ...
                    print(f"  + Inserted well {s['well_name']} ({uwi})")
                except Exception as e:
                    skipped.append(f"{s['well_name']} — could not insert well: {e}")
                    continue

            survey_id = hashlib.sha1(f"{uwi}_SURVEY_PDF".encode()).hexdigest()[:40]

            # Optionally wipe existing
            if wipe_existing:
                con.execute(text(
                    "DELETE FROM dataview.dv_well_dir_srvy_sta WHERE uwi=:u AND survey_id=:sid"
                ), {"u": uwi, "sid": survey_id})
                con.execute(text(
                    "DELETE FROM dataview.dv_well_dir_srvy_hdr WHERE uwi=:u AND survey_id=:sid"
                ), {"u": uwi, "sid": survey_id})
            ...
            for seq, (md, inc, azi, tvd, ns, ew, dls) in enumerate(stations, start=1):
                sta_id = hashlib.sha1(f"{survey_id}_{seq}".encode()).hexdigest()[:40]
                try:
                    con.execute(text("""
                        IF NOT EXISTS (SELECT 1 FROM dataview.dv_well_dir_srvy_sta
                                       WHERE uwi=:u AND survey_id=:sid AND station_id=:staid)
                    ...
                    """), {
                        "u": uwi, "sid": survey_id, "staid": sta_id,
