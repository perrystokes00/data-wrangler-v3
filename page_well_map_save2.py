...
def _add_viewport_wells(m, df, viewport_uwis):
    ...
    for _i, row in sub.iterrows():
        ...
        popup_html = (
            ...
            f"<span style='font-family:monospace;font-size:10px;color:#888'>{uwi}</span>"
            f"<hr style='margin:4px 0'>"
            f"<small>{op} &middot; {field}<br>"
            f"<b style='color:{color}'>{status}</b>"
            f" &middot; Spud {spud} &middot; TD {td}</small>"
            f"<hr style='margin:4px 0'>"
            f"<span style='font-size:12px;color:#1a73e8;font-weight:600'>"
                ...
                try:
                    if gtype == "Polygon" and coords:
                        ...
                        if selected:
                            ...
                            # Also add to tray for batch reporting (de-duped)
                            ...
                            st.success(
                                ...
                                f"individual markers · {len(new_uwis):,} new in tray"
                            )
                            st.rerun()
                        else:
