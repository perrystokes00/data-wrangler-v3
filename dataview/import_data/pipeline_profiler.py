...
Wrap your OWN stages like this (scan, catalog, capture, vault, enrich, …):
    from dataview.import_data.pipeline_profiler import Profiler
    prof = Profiler(cur)
    with prof.stage("scan"):     run_scan(...)
    with prof.stage("catalog"):  run_catalog(...)
    with prof.stage("capture"):  run_capture(...)
    prof.report()
    from dataview.import_data.pipeline_profiler import analyze; analyze(cur, since)
"""
...
def run_promote_whole(conn, cur, prof, apply):
    from dataview.file_catalog.promote_catalog import run_promote
...
def run_promote_per_table(conn, cur, prof, apply):
    from dataview.file_catalog.promote_catalog import (discover_tables, promote_table,
