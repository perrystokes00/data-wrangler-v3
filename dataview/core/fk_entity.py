...
# ═══════════════════════════════════════════════════════════════════════
# SERVER-BACKED FRAME ACCESSOR
# ═══════════════════════════════════════════════════════════════════════

class ServerFrame:
    """
    Minimal, lazy, server-backed stand-in for a staging DataFrame.

    build_fk_graph touches the staging frame only two ways: `.columns` and
    `frame[col].dropna().unique()`. This accessor answers both from the server
    and — critically — holds ONE connection open for its lifetime, the way the
    batch loader does. Opening a fresh connection per query is what made this
    slow: if the DB has AUTO_CLOSE on, each new connection reopens it. One held
    connection keeps the DB open, so only the first query can pay that cost.
    """
    def __init__(self, engine, schema: str, table: str):
        self._engine = engine
        self._schema = schema
        self._table  = table
        self._conn = None
        ...
        if self._cols is None:
            ...
            try:
                rows = self._connection().execute(text(
                    ...
                    "WHERE object_id = OBJECT_ID(:t) ORDER BY column_id"),
                    {"t": f"{self._schema}.{self._table}"}).fetchall()
    ...
    def __getitem__(self, col: str):
        return _ServerColumn(self, col)

    def _distinct_values(self, col: str) -> list:
        if col not in self._distinct:
            from sqlalchemy import text
            try:
                rows = self._connection().execute(text(
                    f"SELECT DISTINCT [{col}] "
                    f"FROM [{self._schema}].[{self._table}] WITH (NOLOCK) "
                    f"WHERE [{col}] IS NOT NULL")).fetchall()
                self._distinct[col] = [r[0] for r in rows]
            except Exception:
                self._distinct[col] = []
...
class _ServerColumn:
    """Supports the only column ops build_fk_graph uses: .dropna().unique()."""
    def __init__(self, frame: "ServerFrame", col: str):
        self._frame = frame
        self._col   = col

    def dropna(self):
        return self          # NULLs already excluded by the DISTINCT query

    def unique(self):
        return self._frame._distinct_values(self._col)
...
def build_fk_graph(
    ...
    """
    from dataview.core.fk import check_fk_violations, get_existing_parent_values
    ...
    from dataview.core.fk import introspect_fk_constraints
    ...
    def _has_saved_mapping(tbl: str) -> bool:
        ...
        try:
            from dataview.import_data.mapping import _load_cache, _entity_cache_key
            cache = _load_cache()
    ...
    for c in constraints:
        ...
        parent_vals = set()
        if engine:
            # Targeted WHERE IN on just the source values — NOT a full-table
            # scan of the parent. build_fk_graph compares only the first parent
            # column (t[0]), so look up only c.parent_cols[0]; passing all cols
            # of a COMPOSITE FK (e.g. county_name+province_state_id) with single
            # values would match nothing and falsely flag everything.
            _cv = {(v,) for v in src_vals}
            _pcols = [c.parent_cols[0]] if c.parent_cols else c.parent_cols
            existing = get_existing_parent_values(
                engine, c.parent_schema, c.parent_table, _pcols,
                check_values=_cv
            )
...
def build_entity_mapping(
    ...
    """
    from dataview.import_data.mapping import AUDIT_COLUMNS  # reuse audit col list
...
def insert_entity_rows(
    ...
    try:
        from sqlalchemy import text
        from dataview.import_data.mapping import AUDIT_COLUMNS
