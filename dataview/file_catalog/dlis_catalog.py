...
def parse_dlis_header(file_path: str, header_only: bool = False) -> dict:
    """
    ...
    header_only=True skips the per-frame walk entirely (no frame.curves()
    depth read, no channel enumeration) — only origin/well metadata,
    frame_count, and parameters are returned. This is the fast path: the
    frame.curves() call materialises curve data and is what makes DLIS slow.
    """
    result = {
        "logical_files": [],
    ...
    with dlis.load(file_path) as logical_files:
        ...
        for lf_idx, lf in enumerate(logical_files):
            ...
            # Frames + channels
            total_channels = 0
            for frame in ([] if header_only else lf.frames):
                frame_data = {
...
def catalog_dlis_file(engine, file_path: str, repository_id: str,
    ...
    try:
        header = parse_dlis_header(file_path, header_only=header_only)
    except Exception as e:
    ...
    if not effective_uwi:
        ...
        for lf in header["logical_files"]:
            ...
            if effective_uwi:
                break
    if not effective_uwi:
        # Last resort: name parsed from the filename (LIS/DLIS often carry the
        # well identity in the filename, not the origin record). Internal
        # values above always win; this only fills a genuine blank.
        effective_uwi = (well_name_fallback or "").strip()
    if not effective_uwi:
    ...
    for lf in header["logical_files"]:
        ...
        lf_row = {
            ...
            "WELL_NAME":        lf.get("well_name") or (well_name_fallback or None),
...
def parse_lis_header(file_path: str, header_only: bool = False) -> dict:
    """
    Read LIS metadata and channel definitions.
    Returns dict with well metadata and channel list.

    header_only=True skips the data-format-spec walk (lis.curves(), which
    materialises curve data for the depth range and channel list) — only
    wellsite/well metadata is returned. This is the fast path.
    """
    result = {
    ...
    with lis.load(file_path) as lis_files:
        ...
        for spec in ([] if header_only else lf.data_format_specs()):
            try:
...
def catalog_lis_file(engine, file_path: str, repository_id: str,
    ...
    try:
        header = parse_lis_header(file_path, header_only=header_only)
    except Exception as e:
        result["error"] = f"Parse failed: {e}"
        return result

    effective_uwi = (uwi.strip() if uwi else "") or (well_name_fallback or "").strip()
    if not effective_uwi:
        result["error"] = (
    ...
    file_row = {
        ...
        "FILE_SIZE_KB":     file_size_kb,
        "FILE_HASH":        file_hash,
        "WELL_NAME":        header.get("well_name") or (well_name_fallback or None),
