...
def run(engine=None):
    ...
    with mapcol:
        ...
        if drawings:
            # TEMP DIAG — trace circle processing across reruns.
            _diag_hashes = [hash(json.dumps(d.get("geometry", {}), sort_keys=True))
                            for d in drawings]
            _diag_proc = list(st.session_state.processed_drawings)
            st.warning(
                f"DIAG circle — n_drawings={len(drawings)} · "
                f"drawing_hashes={_diag_hashes} · "
                f"already_processed={_diag_proc} · "
                f"viewport_gom_now={len(st.session_state.get('viewport_gom_wells', []))}"
            )
