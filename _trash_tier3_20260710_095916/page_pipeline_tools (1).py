...
# stage code lives in these modules; pipeline_run imports them lazily at call
# time, so reloading them in sys.modules before a run picks up on-disk edits
# WITHOUT restarting Streamlit — the whole point of the harness.
_STAGE_MODULES = ("enrich_file_headers", "vault_organizer", "promote_catalog",
                  "worker_core", "build_catalog_mirror")


def _reload_code():
    """Reload stage modules + pipeline_run from disk so the latest patch is live."""
    global pr
    for m in _STAGE_MODULES:
        ...
        if mod is not None:
            try:
                ...
                st.warning(f"reload {m} failed: {e}")
    try:
        pr = importlib.reload(pr)
    except Exception as e:
        st.warning(f"reload pipeline_run failed: {e}")
...
def _run(label, fn):
    ...
    _reload_code()                     # pick up any on-disk patch before running
    logs = []
...
def render():
    ...
    st.caption("Run one stage at a time against the current catalog — fast "
               "iteration, no full pipeline run. Stage code is reloaded from "
               "disk on every run, so patches take effect without restarting.")
    try:
        eng, src = _get_engine()
    except Exception as e:
