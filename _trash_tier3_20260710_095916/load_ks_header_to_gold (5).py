...
for r in cur.execute(
    ...
    # max_length is bytes; nvarchar/nchar are 2 bytes/char, -1 = MAX
    n = r.max_length
    if r.typ in ("nvarchar", "nchar") and n > 0:
        n = n // 2
    _gold_len[r.name.lower()] = n
_gold_cols = set(_gold_len)
def _has(c):
    return c.lower() in _gold_cols
def _fitlit(col, literal):
    """For a quoted string literal, LEFT()-truncate it to the column's width."""
    m = _gold_len.get(col.lower())
    if m and m > 0 and literal.startswith("'") and literal.endswith("'"):
        return f"LEFT({literal}, {m})"
    return literal
...
_use = [(c, _fitlit(c, e)) for (c, e) in _pairs if _has(c)]
