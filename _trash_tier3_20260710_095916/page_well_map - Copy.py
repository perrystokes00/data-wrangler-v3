...
# page still runs.
try:
    from petroleum_regions import PETROLEUM_REGIONS
...
AREAS = [
    ...
    {"label": "🗂 dataview",         "id": "main",       "sources": ["main"],
     ...
     "queries": ["all", "operator", "well_type", "source", "area",
...
def _qry_wells_bcp(_engine, _v: int = 1, limit: int = 10000,
    ...
    (500K+ wells) without the user explicitly raising the well-count slider.
...
def _qry_gom_well_grid(_engine, step: float = 0.36) -> pd.DataFrame:
    ...
    Transport: tries BCP first (sub-second), falls back to pyodbc if BCP
    isn't available. Mirrors the BCP-bypass added to _qry_well_grid /
    _qry_h3_grid to avoid pyodbc transport bottlenecks on cross-schema
    operations.
    ...
    """
    # ── BCP queryout SQL (inline step value, no parameter binding) ──
    _step_lit = f"{step:.6f}"
    bcp_sql = f"""
        SELECT
    ...
    work_dir = Path(os.environ.get("LOCALAPPDATA", "/tmp")) / "Temp" / "dw_gom_grid_bcp"
    try:
        work_dir.mkdir(parents=True, exist_ok=True)
        csv_path = work_dir / f"gom_grid_{step:.4f}.csv"
        ...
        try:
            ...
            # Empty result — fall through to pyodbc just in case.
        except Exception as bcp_exc:
            print(f"[gom_grid] BCP path failed: {bcp_exc}; falling back to pyodbc")
            try:
                csv_path.unlink()
            except FileNotFoundError:
                pass
...
# ══════════════════════════════════════════════════════════════════════════════
# H3 hex grid renderer (Session 3)
# ══════════════════════════════════════════════════════════════════════════════
def _add_h3_layer(
    m,
    df: pd.DataFrame,
    ...
    count. Mirrors _add_well_grid's color logic and selected/interactive
    semantics so behavior is consistent across the two aggregate modes.

    Cells in `selected_set` (set of h3 cell IDs) get a bold blue outline,
    matching the multi-select Commit pattern from Grid mode.
...
def _load_well_geojson(geojson_path: str = "wells.geojson") -> dict:
    """Load a wells GeoJSON file into session state.

    Caches per file path so switching files reloads the data.
    """
    _key = f"_well_geojson_{geojson_path}"
...
def _filter_geojson(geojson: dict, schema: str = "all",
                    area: str = "", source: str = "",
                    county: str = "") -> dict:
    """Filter a GeoJSON FeatureCollection by schema, area, source, county.
    ...
    for f in features:
        ...
        if source and props.get("source", "") != source:
            continue
        if county and props.get("county", "") != county:
            continue
...
# Drill-flow cap: when a rectangle/circle/cell-Commit returns more than this
# many wells, we don't auto-fill the tray — instead the user gets a confirm
# button. Below the cap, wells go straight into the tray. Keeps the per-well
# checkbox UI manageable.
_TRAY_AUTO_ADD_CAP = 500


def _add_drill_results_to_tray(wells: list[dict]) -> int:
    """
    Append drill-result wells to the tray (`clicked_uwis`) and shadow cache
    (`tray_well_data`). Drill results are full well dicts (uwi + well_name,
    operator_name, etc.) — unlike _add_to_tray which expects GeoJSON shape.

    Returns the count actually added (excluding duplicates already in tray).
    """
    if not wells:
        return 0
    if "clicked_uwis" not in st.session_state:
    ...
    for w in wells:
        ...
        if uwi not in st.session_state.clicked_uwis:
            st.session_state.clicked_uwis.append(uwi)
            _added += 1
        # Update shadow with the full well dict so the tray label, scout
...
def _add_wells(m, df, exclude_uwis=None, disable_clustering_at_zoom=13):
    ...
    - Clusters auto-uncluster at the zoom level specified by
      `disable_clustering_at_zoom` (default 13 = township-level detail)
    - Individual markers also come from the user drawing a rectangle viewport,
    ...
    Args:
        ...
        disable_clustering_at_zoom: zoom level at or above which clusters
            dissolve into individual markers. 13 = township-level (default,
            ~100-500 wells/view in dense KS counties). Lower values uncluster
            sooner (riskier for performance in dense areas); higher values
            stay clustered longer (safer but less detailed).
    """
    if df.empty:
        return
    ...
    callback = """
        function(row) {
            ...
            // Tooltip: well name + operator + status, joined with line breaks.
            // Each field has an explicit empty-check so we never show empty
            // lines or 'undefined'. Falls back to UWI if name is empty.
            var ttName = (name && name.trim()) ? name : uwi;
            var ttOp   = (op   && op.trim()   && op   !== '—') ? op   : '';
            var ttStat = (status && status.trim()) ? status : '';
            var ttLines = ['<b>' + ttName + '</b>'];
            if (ttOp)   ttLines.push(ttOp);
            if (ttStat) ttLines.push('<i>' + ttStat + '</i>');
            marker.bindTooltip(
                ttLines.join('<br>'),
    ...
    try:
        ...
        FastMarkerCluster(
            ...
            name="🛢 Wells (clusters)",
            options={
                # Zoom-based uncluster: clusters break into individual markers
                # at the threshold passed in via disable_clustering_at_zoom.
                # Default 13 (township-level — typical view shows 100-500 wells
                # in dense KS counties, render-safe). Below this zoom clusters
                # stay together as density indicators.
...
def run(engine=None):
    ...
    # later.
    #
    # Picking an Advanced Filter (TD, spud date, completion date) also
    # triggers wells-load — the user wants to see the filter take
    # effect immediately.
    ...
    _adv_filters_active = (
        st.session_state.get("wm_adv_td_op",   "ignore") != "ignore"
        or st.session_state.get("wm_adv_spud_op", "ignore") != "ignore"
        or st.session_state.get("wm_adv_comp_op", "ignore") != "ignore"
    )
...
# ── Resolve query filter for SQL push-down ─────────────────────
    ...
    _QUERY_MAP = {"By source":"source", "By operator":"operator",
    ...
    st.session_state["_active_where_extra"] = _qry_where
    ...
    if _need_wells:
        ...
        try:
            ...
            _well_limit = int(st.session_state.get("wells_limit", 10000))
            ...
            if (not _use_gom_wells
                ...
                _prog_msg.warning(
                    ...
                    f"{_near_phrase}. More exist — raise 'Max wells to load' "
                    f"or use Grid/H3 mode for a full density overview."
                )
            else:
                _prog_msg.success(
    ...
    # ── Top bar above map: Background | Area | Zoom to | Query | Database ──
    # Order follows "scope from outside-in" — set the visual context
    # (Background), then the regional scope (Area), then where to look
    # (Zoom to), then what to ask (Query). Database is last because
    # it's a set-once-per-session outer scope that rarely changes.
    top1, top2, top3, top4, top5 = st.columns([1, 1, 1, 2, 1])
    with top1:
    ...
    with top4:
        ...
        QUERIES = {
            "All wells":None,"By operator":"operator",
        ...
        _query_labels = [
            ...
            ["all","operator","well_type","source","area",
             "has_tops","has_prod","has_dst","has_survey",
             "has_core","has_petro"]
        ...
        if qtype in ("operator", "well_type", "source", "area",
    ...
    with top5:
        # Database selector — outer scope, set-once-per-session. Stays
        # at the end of the row because users rarely change it after
        # initial setup.
        _db_options = ["DataView", "DataView_Test", "wrangler"]
        _map_db = st.selectbox("📊 Database", _db_options, key="wm_map_db",
                                help="Which database to query. Set once per "
                                     "session; rarely changes.")
        if _map_db != "DataView":
            try:
                from dw_utils import make_engine
                engine = make_engine(_map_db)
            except Exception as _e:
                st.error(f"Cannot connect to {_map_db}")
    ...
    with ctrl:
        ...
        if len(_combined_regions) > 1:
            st.caption("📍 Region")
            region_sel = st.selectbox(
                "Region",
                list(_combined_regions.keys()),
                index=0,
                key="wm_region",
                label_visibility="collapsed",
                help="Optional. 🏔 entries are industry-canonical "
                     "petroleum plays (Permian, Eagle Ford, etc.). "
                     "📍 entries are your own state regions from the "
                     "Region Builder page. Combines with other filters.",
            )
        else:
            st.caption(
                "💡 *Region selector appears here once "
                "`petroleum_regions.py` or `state_regions.py` is in V3 root.*"
            )
            region_sel = "— none —"
        ...
        # Wrap the whole status block in an expander. The summary header
        # ("🚦 Status (3/6)") is rendered inside the expander label so the
        # collapsed state still shows the active count at a glance. Default
        # is COLLAPSED — clutter reduction. Users who actively need it can
        # open it and the checkboxes show with their full layout.
        with st.expander(f"🚦 Status ({n_s}/{len(all_statuses)})",
                         expanded=False):
            sa, sb = st.columns(2)
        ...
        # ── Advanced Filters ────────────────────────────────────────
        # Comparison-based filters for numeric/date columns that don't
        # fit the categorical Query dropdown pattern. Three filters
        # ship today: TD, spud date, completion date. Each is
        # independent — leave it at "ignore" to disable that filter.
        #
        # Active filters live in session state under keys like:
        #   wm_adv_td_op       — comparison operator ("ignore", ">", "<", ...)
        #   wm_adv_td_val      — numeric value
        #   wm_adv_td_val2     — second value (for "between")
        # (same shape for spud_date and completion_date)
        #
        # The mask handler reads these keys directly. Empty filters
        # ("ignore") are no-ops, so the user can leave the expander
        # in any state without affecting other filtering.
        with st.expander("🔍 Advanced Filters", expanded=False):
            st.caption(
                "Comparison filters for numeric / date columns. "
                "Leave at *ignore* to disable. Composes with "
                "Query / Status / Region / Zoom-to."
            )

            _ops_num  = ["ignore", ">", ">=", "<", "<=", "=", "between"]
            _ops_date = ["ignore", "after", "before", "between"]

            # --- TD ---
            st.markdown("**Total depth (final_td)**")
            _td_op = st.selectbox(
                "TD operator", _ops_num,
                index=0, key="wm_adv_td_op",
                label_visibility="collapsed",
            )
            if _td_op == "between":
                ...
                with _tdc1:
                    st.number_input(
                        "Min TD", min_value=0, max_value=50000,
                        value=int(st.session_state.get("wm_adv_td_val") or 0),
                        step=500, key="wm_adv_td_val",
                        label_visibility="collapsed",
                    )
                with _tdc2:
                    st.number_input(
                        "Max TD", min_value=0, max_value=50000,
                        value=int(st.session_state.get("wm_adv_td_val2") or 20000),
                        step=500, key="wm_adv_td_val2",
                        label_visibility="collapsed",
                    )
            elif _td_op != "ignore":
                st.number_input(
                    "TD value (ft)", min_value=0, max_value=50000,
                    value=int(st.session_state.get("wm_adv_td_val") or 10000),
                    step=500, key="wm_adv_td_val",
                    label_visibility="collapsed",
                )
            ...
            # --- Spud date ---
            st.markdown("**Spud date**")
            _spud_op = st.selectbox(
                "Spud op", _ops_date,
                index=0, key="wm_adv_spud_op",
                label_visibility="collapsed",
            )
            if _spud_op == "between":
                ...
                with _sdc1:
                    st.date_input(
                        "From", key="wm_adv_spud_val",
                        label_visibility="collapsed",
                    )
                with _sdc2:
                    st.date_input(
                        "To", key="wm_adv_spud_val2",
                        label_visibility="collapsed",
                    )
            elif _spud_op != "ignore":
                st.date_input(
                    "Spud date", key="wm_adv_spud_val",
                    label_visibility="collapsed",
                )
            ...
            _comp_op = st.selectbox(
                "Comp op", _ops_date,
                index=0, key="wm_adv_comp_op",
                label_visibility="collapsed",
            )
            if _comp_op == "between":
                ...
                with _cdc1:
                    st.date_input(
                        "From ", key="wm_adv_comp_val",
                        label_visibility="collapsed",
                    )
                with _cdc2:
                    st.date_input(
                        "To ", key="wm_adv_comp_val2",
                        label_visibility="collapsed",
                    )
            elif _comp_op != "ignore":
                st.date_input(
                    "Completion date", key="wm_adv_comp_val",
                    label_visibility="collapsed",
                )

            # Visual indicator when any filter is active
            _adv_active = (
                _td_op != "ignore"
                or _spud_op != "ignore"
                or _comp_op != "ignore"
            ...
            if _adv_active:
                st.success("✓ Advanced filters active")
    ...
    with mapcol:
        ...
        if wells_df.empty:
            ...
            # ── Advanced Filters (TD / spud date / completion date) ──
            # Comparison-based filters set by the Advanced Filters
            # expander in the left panel. Each filter has an operator
            # ("ignore" disables it) and one or two values. NULL column
            # values fail comparison (pandas semantics) — wells with
            # unknown TD don't pass a "TD > X" filter, which is the
            # honest behavior.
            #
            # TD is numeric (final_td column). Dates are strings in
            # YYYY-MM-DD format from the SQL CONVERT — string
            # comparison works because ISO-8601 sorts lexicographically.
            # Converting to actual dates here would also work but adds
            # overhead for no gain.

            # TD filter
            _td_op_v = st.session_state.get("wm_adv_td_op", "ignore")
            if _td_op_v != "ignore" and "final_td" in wells_df.columns:
                _td_v1 = st.session_state.get("wm_adv_td_val")
                _td_v2 = st.session_state.get("wm_adv_td_val2")
                _td_col = pd.to_numeric(wells_df["final_td"], errors="coerce")
                if _td_op_v == ">"  and _td_v1 is not None:
                    mask &= _td_col >  _td_v1
                elif _td_op_v == ">=" and _td_v1 is not None:
                    mask &= _td_col >= _td_v1
                elif _td_op_v == "<"  and _td_v1 is not None:
                    mask &= _td_col <  _td_v1
                elif _td_op_v == "<=" and _td_v1 is not None:
                    ...
                    mask &= _td_col == _td_v1
                elif (_td_op_v == "between"
                      and _td_v1 is not None and _td_v2 is not None):
                    _lo, _hi = sorted([_td_v1, _td_v2])
                    mask &= (_td_col >= _lo) & (_td_col <= _hi)

            # Spud date filter
            _spud_op_v = st.session_state.get("wm_adv_spud_op", "ignore")
            if _spud_op_v != "ignore" and "spud_date" in wells_df.columns:
                _spud_v1 = st.session_state.get("wm_adv_spud_val")
                _spud_v2 = st.session_state.get("wm_adv_spud_val2")
                _spud_col = wells_df["spud_date"].fillna("").astype(str)
                if _spud_v1 is not None:
                    _spud_v1s = str(_spud_v1)
                    if _spud_op_v == "after":
                        mask &= (_spud_col > _spud_v1s) & (_spud_col != "")
                    elif _spud_op_v == "before":
                        mask &= (_spud_col < _spud_v1s) & (_spud_col != "")
                    elif _spud_op_v == "between" and _spud_v2 is not None:
                        _spud_lo, _spud_hi = sorted([str(_spud_v1), str(_spud_v2)])
                        mask &= (
                            (_spud_col >= _spud_lo)
                            & (_spud_col <= _spud_hi)
                            & (_spud_col != "")
            ...
            _comp_op_v = st.session_state.get("wm_adv_comp_op", "ignore")
            if _comp_op_v != "ignore" and "completion_date" in wells_df.columns:
                _comp_v1 = st.session_state.get("wm_adv_comp_val")
                _comp_v2 = st.session_state.get("wm_adv_comp_val2")
                _comp_col = wells_df["completion_date"].fillna("").astype(str)
                if _comp_v1 is not None:
                    _comp_v1s = str(_comp_v1)
                    if _comp_op_v == "after":
                        mask &= (_comp_col > _comp_v1s) & (_comp_col != "")
                    elif _comp_op_v == "before":
                        mask &= (_comp_col < _comp_v1s) & (_comp_col != "")
                    elif _comp_op_v == "between" and _comp_v2 is not None:
                        _comp_lo, _comp_hi = sorted([str(_comp_v1), str(_comp_v2)])
                        mask &= (
                            (_comp_col >= _comp_lo)
                            & (_comp_col <= _comp_hi)
                            & (_comp_col != "")
        ...
        with _mode_col:
            ...
            _new_mode = st.radio(
                "Map mode",
                options=["none", "grid", "h3", "wells", "geojson"],
                index={"none":0,"grid":1,"h3":2,"wells":3,"geojson":4}.get(_current_mode,0),
            ...
            if _new_mode != _current_mode:
                ...
                if _new_mode in ("grid", "h3"):
                    st.session_state["viewport_uwis"] = []
                    # Also clear cell selections — they're mode-specific
                    # (rectangular keys vs h3 cell IDs aren't comparable).
                    st.session_state["selected_cells"] = []
                    st.session_state.pop("_last_grid_click", None)
                # Switching INTO h3 also wipes GOM-specific drill state
                # since H3 doesn't use the GOM Commit/marker path —
                # otherwise stale amber-ring markers from a previous
                # Grid-mode GOM drill linger on top of the H3 hexes.
                if _new_mode == "h3":
        ...
        with _mode_help:
            if _new_mode == "grid":
                ...
                # ── Well-count limit slider (Wells mode only) ───────────
                # Guardrail so the loader never pulls more wells than this
                # without the user deliberately raising it. Default 10,000.
                # The value is read at load time (see _qry_wells_bcp call)
                # and injected as TOP (n) in the SQL — so the cap is enforced
                # server-side, not just at render. Persisted like the
                # uncluster slider: session_state during the run,
                # user_prefs.json across restarts.
                #
                # The slider tops out at 100,000. We intentionally do NOT
                # offer "unlimited" as a one-click option — pulling all 500K+
                # wells is the exact thing this guardrail exists to prevent.
                # A user who truly wants everything can set it to 100K (which
                # covers the vast majority of real working sets) or change
                # the default in code.
                if "wells_limit" not in st.session_state:
                    _prefs = _load_user_prefs()
                    st.session_state["wells_limit"] = int(
                        _prefs.get("wells_limit", 10000)
                    )
                _well_lim = st.slider(
                    "Max wells to load",
                    min_value=1000,
                    max_value=100000,
                    value=st.session_state["wells_limit"],
                    step=1000,
                    key="wells_limit_slider",
                    help=(
                        "Caps how many wells the map will load at once. "
                        "Default 10,000. Raise it only if you need more and "
                        "your machine can handle the render. The cap is "
                        "enforced in the SQL query (TOP n ORDER BY uwi), so "
                        "lowering it makes loads faster. Saved between sessions."
                    ),
                )
                if _well_lim != st.session_state["wells_limit"]:
                    st.session_state["wells_limit"] = _well_lim
                    _prefs = _load_user_prefs()
                    _prefs["wells_limit"] = _well_lim
                    _save_user_prefs(_prefs)
                    # Bust the wells cache so the new limit takes effect, then
                    # rerun. Without clearing, the cached result for the old
                    # limit would be returned (limit is part of the cache key,
                    # so actually a fresh limit re-queries automatically — but
                    # we clear the "already loaded" flag so the loader re-runs).
                    st.session_state["_wells_already_loaded"] = False
                    # Changing the limit is a deliberate "load wells" action,
                    # so lift any "✗ Clear wells" suppression.
                    st.session_state["wells_suppressed"] = False
                    st.rerun()

                # ── Uncluster zoom slider (Wells mode only) ─────────────
                # Lets the user choose the zoom level at which clusters dissolve
                # into individual markers. Lower = uncluster sooner (more detail
                # at lower zoom, potential lag in dense areas). Higher = stay
                # clustered longer (safer, less detail).
                #
                # Persistence: session_state during the run, user_prefs.json
                # across Streamlit restarts. The disk file is read once per
                # session (on first slider render) and written every time the
                # user changes the slider value.
                if "uncluster_zoom" not in st.session_state:
                    # First read this session — pull from disk if available
                    _prefs = _load_user_prefs()
                    st.session_state["uncluster_zoom"] = int(
                        _prefs.get("uncluster_zoom", 13)
                    )
                _cluster_z = st.slider(
                    "Uncluster at zoom",
                    min_value=8,
                    max_value=18,
                    value=st.session_state["uncluster_zoom"],
                    step=1,
                    key="uncluster_zoom_slider",
                    help=(
                        "At or above this zoom level, cluster bubbles break "
                        "into individual well markers. Try 11–12 for sparse "
                        "areas, 13–14 for dense Kansas counties, 15+ for "
                        "very dense plays. Saved between sessions."
                    ),
                )
                if _cluster_z != st.session_state["uncluster_zoom"]:
                    st.session_state["uncluster_zoom"] = _cluster_z
                    # Merge-and-save so we don't clobber other future prefs.
                    _prefs = _load_user_prefs()
                    _prefs["uncluster_zoom"] = _cluster_z
                    _save_user_prefs(_prefs)
                    st.rerun()
                ...
                if st.button(
                    ...
                    help=(
                        ...
                        "(rectangle / circle), the base cluster layer, AND "
                        "the Object Tray — leaving just the basemap. Load "
                        "wells again by re-selecting an area or adjusting the "
                        "well-count slider."
                    ),
                ):
                    st.session_state["viewport_uwis"] = []
            ...
            with c3:
                ...
                if st.button(
                    ...
                    if _total_drilled:
                        ...
                        if _total_drilled <= _TRAY_AUTO_ADD_CAP:
                            _added = _add_drill_results_to_tray(
                                list(_drilled_main) + list(_drilled_gom)
                            )
                            st.session_state.pop("_pending_drill_wells", None)
                            st.success(
                                f"📐 Added **{_added:,}** wells to the Object "
                                f"Tray from {_sel_n} cell(s) "
                                f"({' + '.join(_parts)} drilled). Grid hidden "
                                "— toggle 'Show grid' to select more."
                            )
                        else:
                            ...
                            st.session_state["_pending_drill_label"] = (
                                f"{_sel_n} cell(s) ({' + '.join(_parts)})"
                            )
                            st.info(
                                f"📐 Drill returned **{_total_drilled:,}** "
                                "wells — above the auto-add cap of "
                                f"{_TRAY_AUTO_ADD_CAP:,}. Use the prompt "
                                "below the map to confirm or pick fewer cells."
                            )
                        st.rerun()
                    else:
        ...
        # without losing them.
        if _new_mode == "h3":
            _h3_c1, _h3_c2 = st.columns([3, 5])
            with _h3_c1:
                _h3_res_choice = st.select_slider(
                    "H3 Resolution",
                    options=[4, 5, 6, 7],
                    value=int(st.session_state.get("h3_resolution", 5)),
                    key="h3_resolution_slider",
                    help=(
                ...
                if _h3_res_choice != st.session_state.get("h3_resolution", 5):
                    st.session_state["h3_resolution"] = int(_h3_res_choice)
                    # Clear stale click dedupe — different resolution
                    # means clicking the same coords resolves to a
                    # different hex now.
                    st.session_state.pop("_last_h3_click", None)
                    st.rerun()
            with _h3_c2:
                st.caption(
                    f"🔶 Currently rendering R{_h3_res_choice} hexes. "
                    f"Click any hex to drill its wells "
                    f"(up to 5,000 per drill)."
                )
        ...
        if _viewport_bounds is None:
            ...
            elif st.session_state.get("map_mode", "grid") == "h3":
        ...
        if _map_mode == "grid":
            ...
            if _show_main_grid:
                ...
                if not active_area.get("sources", []):
                    _msg.info("🔵 Pick a schema in the dropdown above to see the density grid.")
                else:
            ...
            # Resolution: read from session_state, default R5. The user
            # can adjust via the H3 controls below (rendered after the
            # mode radio). For zoom-adaptive resolution we'd need to
            # subscribe to st_folium's "zoom" return — deferred since
            # the page architecture intentionally avoids those reruns.
            _h3_res = int(st.session_state.get("h3_resolution", 5))
            ...
            if _show_h3:
                ...
                try:
                    ...
                    # If H3 fails, fall back to Grid mode (proven path)
            ...
            # Drilled wells overlay (same as Grid mode — yellow markers
            # on top of the density layer for cells the user has
            # already committed)
            _viewport_uwis = st.session_state.get("viewport_uwis", [])
            if _viewport_uwis:
                try:
            ...
            # Scan for available GeoJSON files
            from pathlib import Path as _P
            _gj_dir = _P(__file__).parent
            _gj_subdir = _gj_dir / "geojson"
            _gj_files = sorted(_gj_subdir.glob("wells*.geojson")) if _gj_subdir.exists() else []
            # Fallback to root dir
            if not _gj_files:
                _gj_files = sorted(_gj_dir.glob("wells*.geojson"))
            if not _gj_files:
                _msg.warning("No wells*.geojson files found. Run build_county_geojson.py.")
            _gj_labels = {f.stem.replace("wells_","").replace("_"," ").title(): str(f) for f in _gj_files}
            if _gj_files:
                _gj_pick = st.selectbox("GeoJSON file",
                    list(_gj_labels.keys()), key="gj_file_pick")
                _gj_path = _gj_labels[_gj_pick]
            else:
                _gj_path = "wells.geojson"
            _full_gj = _load_well_geojson(_gj_path)
            _total_wells = len(_full_gj.get("features", []))
            if _total_wells > 100_000:
                st.caption(
                    f"📊 {_total_wells:,} wells in file. "
                    f"Apply a filter below (max 100K display).")
            ...
            if not _all_features:
                ...
                # County list from features
                _counties = sorted({f["properties"].get("county","")
                                    for f in _all_features} - {""})

                _gj_c1, _gj_c2, _gj_c3, _gj_c4 = st.columns(4)
                ...
                _gj_county = _gj_c4.selectbox("County",
                    ["— All —"] + _counties, key="gj_county")
                ...
                _filtered = _filter_geojson(_full_gj,
                    ...
                    county="" if _gj_county == "— All —" else _gj_county)
                _feats = _filtered.get("features", [])
                _n = len(_feats)

                # Cap at 100K wells to prevent browser crash
                _MAX_WELLS = 500_000
                if _n > _MAX_WELLS:
                    st.warning(
                        f"⚠️ {_n:,} wells exceeds the {_MAX_WELLS:,} display limit. "
                        f"Apply a filter (Area, Source, or County) to narrow the results.")
                    _feats = []
                    _n = 0
                ...
                if _n:
                    ...
                    # Build pydeck data with symbol for IconLayer
                    # Fallback color for ScatterplotLayer backup
                    _STATUS_COLORS = {
                    ...
                    # Load icon mapping
                    _icon_map_path = _P(__file__).parent / "well_icons" / "icon_mapping.json"
                    _icon_atlas_path = _P(__file__).parent / "well_icons" / "well_icons_atlas.png"
                    _use_icons = _icon_map_path.exists() and _icon_atlas_path.exists()

                    if _use_icons:
                        import json as _json
                        _icon_mapping = _json.loads(_icon_map_path.read_text())
                        _atlas_url = str(_icon_atlas_path)
                    ...
                    for _f in _feats:
                        ...
                        _sym = _p.get("symbol", "default_default")
                        # Validate symbol exists in mapping
                        if _use_icons and _sym not in _icon_mapping:
                            _sym = "default_default"
                        _dk_data.append({
                            "lon": _c[0], "lat": _c[1],
                            "uwi": _p.get("uwi",""),
                            "name": _p.get("name",""),
                    ...
                    if _use_icons:
                        _layer = pdk.Layer("IconLayer",
                            data=_dk_data,
                            get_position=["lon", "lat"],
                            get_icon="symbol",
                            icon_atlas=_atlas_url,
                    ...
                    if _dk_event and _dk_event.selection:
                        ...
                        _idx_list = (_sel_indices.get("IconLayer", []) or _sel_indices.get("ScatterplotLayer", [])
        ...
        elif not dff.empty and not st.session_state.get("wells_suppressed", False):
            # Wells mode — needs the full wells dataframe (lazy-loaded
            # when user picked Wells mode or used a filter).
            # The wells_suppressed flag (set by "✗ Clear wells") hides the
            # base cluster layer until wells are deliberately reloaded.
            ...
            _viewport_uwis = st.session_state.get("viewport_uwis", [])
            # Uncluster threshold: session_state first (set by the slider),
            # then disk prefs (persists across sessions), then default 13.
            if "uncluster_zoom" in st.session_state:
                _uncluster_z = st.session_state["uncluster_zoom"]
            else:
                _uncluster_z = int(_load_user_prefs().get("uncluster_zoom", 13))
                st.session_state["uncluster_zoom"] = _uncluster_z
            _add_wells(m, dff,
                       exclude_uwis=_viewport_uwis,
                       disable_clustering_at_zoom=_uncluster_z)

        else:
            # Wells mode but base layer not shown — either no wells loaded
            # yet, or "✗ Clear wells" suppressed the cluster layer. That's
            # fine; the viewport/drilled wells render in their own block
            # below regardless, so a fresh rectangle/circle drill still
            # shows even on a cleared/empty base.
            if dff.empty:
                _msg.info("📍 Wells mode — draw a rectangle or circle to drill wells")

        # ── Viewport / drilled wells — ALWAYS render in wells mode ──────────
        # Independent of the base-cluster suppression above. A rectangle or
        # circle drill populates viewport_uwis + the tray_well_data shadow;
        # we render those as individual markers whether or not the base
        # cluster layer is shown. Sourcing from the shadow (not dff) means
        # drilled wells display even when the base wells for this area were
        # never loaded (e.g. you zoomed to a fresh area and drew a box).
        if _map_mode == "wells":
            _viewport_uwis = st.session_state.get("viewport_uwis", [])
            print(f"[viewport-render] mode=wells "
                  f"viewport_uwis={len(_viewport_uwis)} "
                  f"shadow={len(st.session_state.get('tray_well_data', {}))} "
                  f"dff_empty={dff.empty} "
                  f"suppressed={st.session_state.get('wells_suppressed', False)}")
            if _viewport_uwis:
                try:
                    ...
                    if not _vp_df.empty:
                        _vp_count = _add_viewport_wells(m, _vp_df, _viewport_uwis)
                        if _vp_count:
        ...
        # GOM wells render gated on Area selector, grid_visible AND mode.
        ...
        # Map-mode gate (Session 3): in H3 mode, the federation density
        # view handles GOM hexes via the dataview_gom schema_filter, so
        # this rectangular-grid path should NOT also render — that would
        # show both shapes overlaid.
        _show_gom_grid = (
            ...
            for drawing in drawings:
                ...
                try:
                    ...
                    if _is_rectangle:
                        ...
                        print(f"[rect-drill] bbox=({_min_lat:.3f},{_min_lon:.3f})"
                              f"-({_max_lat:.3f},{_max_lon:.3f}) "
                              f"main={_total_main} gom={_total_gom} "
                              f"sources={_active_sources}")
                        ...
                        if _total_found == 0:
                            ...
                            if _total_in_drill <= _TRAY_AUTO_ADD_CAP:
                                ...
                                st.session_state.pop("_pending_drill_wells", None)
                                st.success(
                                    f"⬛ Added **{_added:,}** wells to the "
                                    f"Object Tray (from {' + '.join(_parts)} "
                                    "drilled)."
                                )
                            else:
                    ...
                    if _circle_total_found == 0:
                        ...
                        if _total_in_drill <= _TRAY_AUTO_ADD_CAP:
                            ...
                            st.success(
                                f"⭕ Added **{_added:,}** wells to the Object "
                                f"Tray within {_radius_m/1000:.1f} km "
        ...
        if (_map_mode == "h3" and _cells_mode and _coord_click
            ...
            if _h3_click_lat is not None and _h3_click_lon is not None:
                _h3_res_active = int(st.session_state.get("h3_resolution", 5))
                try:
        ...
        # ── Pending drill prompt ───────────────────────────────────────────
        # Drills that returned more than _TRAY_AUTO_ADD_CAP wells stash their
        # results in _pending_drill_wells rather than auto-filling the tray
        # (which would render hundreds of checkboxes). The user confirms or
        # discards here.
        _pending = st.session_state.get("_pending_drill_wells")
        if _pending:
            _label = st.session_state.get("_pending_drill_label", "drill")
            st.warning(
                f"⚠️ **{len(_pending):,} wells** waiting from {_label}. "
                "Adding all to the tray will create that many checkboxes — "
                "consider narrowing the area first."
            )
            _pc1, _pc2, _pc3 = st.columns([2, 2, 1])
            with _pc1:
                if st.button(
                    f"➕ Add all {len(_pending):,} to tray",
                    key="confirm_add_pending",
                    type="primary",
                    use_container_width=True,
                ):
                    _added = _add_drill_results_to_tray(_pending)
                    st.session_state.pop("_pending_drill_wells", None)
                    st.session_state.pop("_pending_drill_label", None)
                    st.success(f"Added {_added:,} wells to tray.")
                    st.rerun()
            with _pc2:
                if st.button(
                    f"➕ Add first {_TRAY_AUTO_ADD_CAP:,} only",
                    key="confirm_add_capped",
                    use_container_width=True,
                ):
                    _added = _add_drill_results_to_tray(
                        _pending[:_TRAY_AUTO_ADD_CAP]
                    )
                    st.session_state.pop("_pending_drill_wells", None)
                    st.session_state.pop("_pending_drill_label", None)
                    st.success(f"Added {_added:,} wells to tray.")
        ...
        with st.expander(tray_label, expanded=True):
            ...
            # ── Search & add — multiselect from filtered + drilled wells ──
            # Searches over _display_wells (filter-aware) merged with any
            # drilled wells from the shadow cache. Picking adds to the tray.
            # Placed inside the tray so the user has one place to manage
            # well selection rather than two parallel UIs.
            _pick_opts = {}
            _seen_uwis_for_picker = set()
            for w in _display_wells:
                _u = w.get("uwi")
                if not _u or _u in _seen_uwis_for_picker:
                    continue
                _seen_uwis_for_picker.add(_u)
                _label_pick = (
                    f"{w.get('well_name','?')} — "
                    f"{w.get('operator_name','')}"
                )
                _pick_opts[_label_pick] = _u
            if _pick_opts:
                ...
                with _pc1:
                    _picked_labels = st.multiselect(
                        ...
                        default=[],
                        key="tray_well_picker",
                        label_visibility="collapsed",
                        placeholder="🔎 Search by well name or operator…",
                    )
                with _pc2:
                    _add_disabled = not _picked_labels
                    if st.button(
                        "➕ Add",
                        key="tray_add_picked",
                        use_container_width=True,
                        ...
                        for lbl in _picked_labels:
                            ...
                            if uwi not in st.session_state.clicked_uwis:
                                st.session_state.clicked_uwis.append(uwi)
                        # Clear the multiselect by popping its key, so the
                        # next render shows it empty for the next search.
                        st.session_state.pop("tray_well_picker", None)
                        st.rerun()
                st.markdown(
                    "<div style='font-size:11px;color:#888;padding:0 0 6px 0'>"
                    "Or click well markers on the map, or draw a circle / "
                    "rectangle to add a region of wells at once."
                    "</div>",
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    "<div style='font-size:11px;color:#888;padding:0 0 6px 0'>"
                    "Click well markers on the map, or draw a circle / "
                    "rectangle to add wells. The search picker will appear "
                    "here once wells are loaded or drilled."
                    "</div>",
                    unsafe_allow_html=True,
                )
            ...
            if not clicked_uwis:
                ...
                st.markdown(
                    ...
                    "<span style='color:#aaa;font-size:12px'>— Empty —&nbsp;&nbsp;"
                    "Search above, click a marker, or draw a selection to add wells."
                    "</span></div>",
                    unsafe_allow_html=True)
            else:
                # Checkbox list — checkboxes are OFF by default. They select
                # which wells get Scout Tickets (Reports). Export Excel
                # ignores them and exports the ENTIRE tray.
                selected_in_tray = []
                ...
                st.markdown(
                    ...
                    "<b>Export Excel</b> exports <b>all</b> wells in the tray. "
                    "<b>Check</b> wells to generate <b>Scout Tickets</b> for "
                    "them (Reports)."
                    "</div>",
                    unsafe_allow_html=True)

                # Action row.
                #   Reports  → CHECKED wells only (scout tickets)
                #   Export   → ENTIRE tray, regardless of checkboxes
                ...
                with p1:
                    if st.button("📋 Reports",
                                 ...
                                 help="Generate Scout Tickets for the CHECKED "
                                      "wells. Check at least one well above."):
                        st.session_state["show_summary"] = True
                        st.session_state["_summary_uwis"] = selected_in_tray
                        st.rerun()
                with p2:
                    # Export operates on the WHOLE tray, not just checked.
                    _all_tray = list(clicked_uwis)
                    if st.button("📊 Export Excel", key="export_xlsx_btn",
                                 use_container_width=True,
                                 disabled=not _all_tray,
                                 help="Export ALL wells in the tray to Excel "
                                      "(checkboxes don't affect export)."):
                        with st.spinner("Building Excel..."):
                            xlsx = _build_export_excel(_all_tray, wells_df, engine)
                        if xlsx:
                            st.download_button(
                                "⬇ Download Excel", data=xlsx,
                                file_name=f"Wells_{len(_all_tray)}.xlsx",
