...
import page_file_inventory_gov
import page_file_workbench
...
with st.sidebar:
    # Bull logo + title — loaded from file at runtime
    import pathlib as _spl, base64 as _sb64
    _s_logo_paths = ["assets/data_wrangler.png", "data_wrangler.png", "assets/cowboy.png"]
    _s_b64 = ""
    for _slp in _s_logo_paths:
        if _spl.Path(_slp).exists():
            _s_b64 = _sb64.b64encode(_spl.Path(_slp).read_bytes()).decode()
            break
    _s_src = f"data:image/png;base64,{_s_b64}" if _s_b64 else ""
    st.markdown(
        f'<div style="display:flex;align-items:center;gap:10px;padding:4px 0 8px 0;">'
        f'<img src="{_s_src}" '
        f'style="width:48px;height:48px;object-fit:cover;border-radius:6px;flex-shrink:0;"/>'
        f'<div>'
        f'<div style="font-size:1.1rem;font-weight:700;color:#C8922A;'
        f'letter-spacing:.05em;line-height:1.1;">DATA WRANGLER</div>'
        f'<div style="font-size:0.65rem;color:#8AABCC;'
        f'letter-spacing:.08em;">PETROLEUM DATA PLATFORM</div>'
        f'</div></div>',
        unsafe_allow_html=True
    )
    if S.demo:
    ...
    with st.expander("📋 Reference Table Manager", expanded=_rtm_expanded):
        # Only mark expanded if explicitly triggered by FK resolution button.
        # Do NOT set True unconditionally — that forces it open on every rerun
        # including during login. State is cleared when user collapses it.
        if _rtm_open_flag:
            st.session_state["_rtm_expanded"] = True
...
def _render_file_catalog_landing():
    ...
    from modules.inv_auth import is_logged_in, current_user

    st.title("📂 Petroleum File Catalog")
    st.caption(
        "Catalog, search, browse and export petroleum files. "
        "Log in to access your role-specific tools."
    )
    st.divider()
    ...
    # Check login state to show role-appropriate cards
    role = current_user()["role"] if is_logged_in() else None

    if not is_logged_in():
        # ── Not logged in — show login prompt + Browse & Copy ─────────────────
        st.info("🔐 Log in to access cataloging and inventory tools.")
        col1, col2 = st.columns(2)
        _domain_card(
            col1, "🔐", "Team Login",
            "Catalogers · Managers · Users",
            "Log in to access your role-specific catalog tools — "
            "cataloging, assignment, search and browse.",
            "Log In →", "fc_login", "file_inventory"
        )
        _domain_card(
            col2, "📋", "Browse & Export",
            "LAS · DLIS · LIS · SEG-Y · P190",
            "Search and preview cataloged files. "
            "Copy files to your local desktop.",
            "Browse & Export →", "fc_browse", "browse"
        )
        return

    # ── Logged in — show role-appropriate cards ───────────────────────────────
    user = current_user()
    st.success(f"👤 **{user['full_name']}** — {user['role']}")

    if role == "USER":
        # End users — browse and export only
        col1, = st.columns(1)
        _domain_card(
            col1, "📋", "Browse & Export",
            "LAS · DLIS · LIS · SEG-Y · P190",
    ...
    # ── CATALOGER / MANAGER / DELEGATE ────────────────────────────────────────
    row1_col1, row1_col2, row1_col3 = st.columns(3)

    _domain_card(
        row1_col1, "🗂", "File Inventory",
        "Scan · Assign · Catalog",
        "Scan network drives to build a file inventory. "
        "Assign files to catalogers and track progress.",
        "Open File Inventory →", "fc_file_inv", "file_inventory"
    )
    _domain_card(
        row1_col2, "🛢️", "Well Logs",
        "LAS · DLIS · LIS",
        "Crawl, catalog and search well log files. "
        "Auto fuzzy-match to PPDM wells. Plot curves.",
    ...
    _domain_card(
        row2_col1, "📋", "Browse & Export",
        "LAS · DLIS · LIS · SEG-Y · P190",
        "Search all cataloged files. Preview headers and plots. "
        "Copy files to a local destination folder.",
        "Browse & Export →", "fc_browse", "browse"
    )

    if role in ("MANAGER", "DELEGATE"):
        _domain_card(
            row2_col2, "📁", "Manage Repositories",
            "Add · Crawl · Inventory",
            "Register repositories, crawl for files, "
            "build inventory and view stats by file type.",
            "Manage Repositories →", "fc_repos", "repos"
        )
        _domain_card(
            row2_col3, "📊", "Manager Dashboard",
            "Metrics · Search · Assignments",
            "Monitor cataloging progress, search inventory and catalog, "
            "view team metrics and login activity.",
            "Open Dashboard →", "fc_mgr", "file_inventory"
        )
    else:
        # Cataloger — show My Work shortcut
        _domain_card(
            row2_col2, "📝", "My Work",
            "Assigned files",
            "View and catalog your assigned files. "
            "Preview headers and plots before cataloging.",
            "My Work →", "fc_mywork", "file_inventory"
        )
...
try:
    if st.session_state.app_mode == "data_model":
        ...
        # File Inventory Governance — requires cataloging team login
        from modules.file_inventory import _detect_dialect
        _inv_dialect = _detect_dialect(S.engine)
        if _inv_dialect == "sqlserver": _inv_dialect = "mssql"
        page_file_inventory_gov.render(S.engine, _inv_dialect)
        ...
        elif _domain == "file_inventory":
            # File Inventory Gov — login gated, role-routed
            from modules.file_inventory import _detect_dialect
            _inv_dialect = _detect_dialect(S.engine)
            if _inv_dialect == "sqlserver": _inv_dialect = "mssql"
            page_file_inventory_gov.render(S.engine, _inv_dialect)
        elif _domain == "browse":
            # Browse & Export — login gated
            from modules.inv_auth import is_logged_in, render_login_screen
            if not is_logged_in():
                render_login_screen(S.engine, "mssql", {})
            else:
                pass
        else:
