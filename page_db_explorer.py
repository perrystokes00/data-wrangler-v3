...
def render(S):
    ...
    with st.container(border=True):
        ...
        # Placeholder example uses a real table from this DB (the first one)
        # rather than a hardcoded dbo.well, which doesn't exist here.
        if table_list:
            _ph_sch, _ph_tbl = (
                table_list[0].split(".", 1) if "." in table_list[0]
                else (_get_schema(engine, _get_dialect(engine)), table_list[0]))
            _ph_sql = _top_sql_for_dialect(engine, _ph_sch, _ph_tbl)
        else:
            _ph_sql = "SELECT TOP 100 * FROM [schema].[table]"
        ...
        if b4.button("TOP 100", use_container_width=True, key="dbx_top100"):
            if st.session_state.dbx_selected:
                ...
                # No table picked — don't guess a (likely non-existent) table.
                st.session_state.dbx_results = None
                st.session_state.dbx_error = (
                    "Pick a table from the dropdown first, then click TOP 100 — "
                    "or type a query and hit Execute.")
            st.rerun()
