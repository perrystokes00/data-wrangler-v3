    ...
    if uwi and engine:
        try:
            ...
            with engine.connect() as _c:
                _row = _c.execute(_t(
                    "SELECT well_name FROM dataview.dv_well WHERE uwi=:u"),
                    {"u": uwi}).fetchone()
            if _row:
                st.success(f"✅ UWI found in DB — **{_row[0] or '—'}**")
            else:
                st.warning(f"⚠️ UWI `{uwi}` not in dataview.dv_well — update above or load well first.")
...
def _extract_by_type(fpath: str, rt: str,
    ...
    try:
        ...
        if rt == RT_DIRECTIONAL:
            ...
            return extract_rft_data(fpath), None
            ...
            return extract_ddr(fpath), None

        elif rt == RT_WELL_TEST:
            return extract_well_test(fpath), None

        elif rt in (RT_PETRO, "PETROPHYSICAL"):
            return extract_petrophysical(fpath), None

        elif rt == RT_EOWR:
            return extract_eowr(fpath), None

        elif rt == RT_CASING:
            return extract_casing_cement(fpath), None
...
def _render_extract(engine, dialect):
    ...
    # ── UWI verification against dv_well ─────────────────────────────
    if uwi and engine is not None:
        try:
            from sqlalchemy import text as _text
            with engine.connect() as _con:
                row = _con.execute(_text(
                    "SELECT uwi, well_name, operator_ba_id "
                    "FROM dataview.dv_well WHERE uwi = :u"
                ), {"u": uwi}).fetchone()
            if row:
                st.success(f"✅ UWI found in DB — Well: **{row[1] or '—'}**")
            else:
                st.warning(
                    f"⚠️ UWI `{uwi}` not found in dataview.dv_well. "
                    "Update the UWI above or load the well record first.")
        except Exception as _e:
            st.caption(f"DB check skipped: {_e}")
