...
_SCAN_OK  = False
_SCAN_ERR = None
# v3 crawl engine — imported if available, falls back to built-in
try:
    from page_dv_catalog import _crawl_and_insert, _CRAWL, _CRAWL_Q, _CRAWL_STOP
    _HAS_V3_CRAWL = True
except ImportError:
    _HAS_V3_CRAWL = False

try:
    from modules.file_inventory import (
...
from modules.inv_auth import (
    is_logged_in, current_user, login, logout, require_role,
    render_login_screen,
)
from modules.inv_email import smtp_configured, test_smtp
...
def _gfc(dialect):
    if dialect == "oracle":    return "dataview.dv_global_file_catalog"
    if dialect == "snowflake": return '"dataview"."dv_global_file_catalog"'
    return "dataview.dv_global_file_catalog"

def _atbl(dialect): return "dataview.dv_catalog_assignment"
def _gtbl(dialect): return "dataview.dv_catalog_group"
def _ftbl(dialect): return "dataview.dv_catalog_file_assignment"
def _utbl(dialect): return "dataview.dv_catalog_user"
...
def render(engine, dialect: str = "mssql"):
    ...
    try:
        ...
        from modules.file_inventory_governance import migrate_group_file_columns
        migrate_group_file_columns(engine, dialect)
        # Pre-create header catalog tables — fast no-op if already exist
        try:
            from modules.file_header_catalog import ensure_header_schema
    ...
    # ── Card navigation ───────────────────────────────────────────────────────
    NAV_ITEMS = [
        ("scan",      "🔍", "Scan",       "Crawl drives and build the file inventory"),
        ("dashboard", "📊", "Dashboard",  "Inventory metrics and duplicate summary"),
        ("mywork",    "📋", "My Work",    "Review, catalog and skip your assigned files"),
        ("progress",  "📈", "Progress",   "Track assignment completion across catalogers"),
        ("shapefiles","🗺️", "Shapefiles", "Scan, classify and load spatial files to DB"),
        ("surveys",   "📄", "PDF Surveys","Extract directional surveys from PDF reports"),
        ("office",    "📊", "Excel / Word","Extract production and completion data"),
        ("admin",     "⚙️", "Admin",      "Manage users, assignments and settings"),
    ]
    ...
    # Single row of all cards
    cols = st.columns(len(visible))
    for col, (key, icon, title, desc) in zip(cols, visible):
            with col:
                is_active = active == key
                border = "2px solid #C8922A" if is_active else "1px solid #e2e8f0"
                title_color = "#C8922A" if is_active else "#1e293b"
                st.markdown(f"""
                <div style="background:#fff;border:{border};border-radius:12px;
                            padding:16px 10px 12px;text-align:center;
                            box-shadow:{'0 0 0 3px #C8922A22' if is_active else '0 1px 3px rgba(0,0,0,0.06)'};
                            margin-bottom:4px;">
                  <div style="font-size:1.8rem;margin-bottom:4px;">{icon}</div>
                  <div style="font-size:0.9rem;font-weight:700;color:{title_color};
                              margin-bottom:3px;">{title}</div>
                  <div style="font-size:0.72rem;color:#64748b;line-height:1.3;margin-bottom:10px;">{desc}</div>
                </div>
                """, unsafe_allow_html=True)
                btn_style = (
                ...
                if st.button(f"{'✓ Active' if is_active else 'Open'} {title}",
                             key=f"nav_{key}",
    ...
    if   active == "scan":      _tab_scan(engine, dialect)
        ...
        try:
            st.info("Shapefile catalog coming soon")
        except ImportError as e:
            st.error(
                f"⚠️ Shapefile support requires geopandas: "
                f"`pip install geopandas fiona` — {e}"
            )
    elif active == "surveys":
        try:
            import page_pdf_catalog
            page_pdf_catalog.run(engine, dialect)
        except ImportError as e:
            st.error(
                f"⚠️ PDF support requires pdfplumber: "
                f"`pip install pdfplumber` — {e}"
            )
    elif active == "office":
        _tab_office_extract(engine, dialect, user)
    ...
    try:
        ...
        if not df.empty:
            for group in sorted(df["FILE_TYPE_GROUP"].unique()):
                ...
                total = int(gdf["file_count"].fillna(0).sum())
                catd  = int(gdf[gdf["catalog_status"]=="CATALOGED"]["file_count"].fillna(0).sum())
...
def _tab_assign(engine, dialect, user, role):
    ...
    if cnt == 0:
        ...
        if st.button("↩ Reset", key="asgn_reset"):
    ...
    with col_reset:
        if st.button("↩ Change filters", key="asgn_reset"):
            st.session_state["asgn_phase"] = 1
        ...
        try:
            ...
            with engine.begin() as conn:
                ...
                for i, uid in enumerate(assignee_ids):
                    ...
                    conn.execute(text(f"""
                        INSERT INTO {at}
                            (assignment_id,group_id,user_id,ASSIGNED_BY,
                    ...
                    if file_rows:
                        conn.execute(text(f"""
                            INSERT INTO {ft}
                                (file_assignment_id,group_id,assignment_id,
...
def _catalog_file_auto(engine, dialect, row, repo_id: str, user: dict):
    ...
    """
    from modules.inv_workbench import mark_cataloged
    from modules.seis_filename_parser import parse_seis_filename
    ...
    if ext == "las":
        ...
        from modules.dlis_catalog import catalog_dlis_file
        r = catalog_dlis_file(engine, fp, repo_id)
    elif ext == "lis":
        from modules.dlis_catalog import catalog_lis_file
...
def _tab_my_work(engine, dialect, user):
    ...
    try:
        from modules.inv_workbench import render_file_workbench
        _wb_ok = True
    except ImportError:
        _wb_ok = False
    ...
    with engine.connect() as conn:
        rows = conn.execute(text(f"""
            SELECT a.ASSIGNMENT_ID, g.GROUP_NAME, g.FILE_TYPE,
                   a.DUE_DATE, a.STATUS, ISNULL(a.FILE_COUNT,0) AS FILE_COUNT
            FROM {at} a
            JOIN {gt} g ON a.group_id = g.group_id
            WHERE a.user_id = :uid AND a.STATUS NOT IN ('COMPLETED','CLOSED')
            ORDER BY a.DUE_DATE
    ...
    for a in assignments:
        ...
        with st.expander(
            ...
            f"{int(a['file_count'] or 0):,} files · Due {due}",
            expanded=False
        ):
            # Get files for this assignment
            with engine.connect() as conn:
                file_rows = conn.execute(text(f"""
                    SELECT f.file_assignment_id, f.INVENTORY_ID,
            ...
            if _wb_ok:
                ...
                if mode == "📋 Batch":
                    ...
                    try:
                        from modules.inv_workbench import _get_repos, _auto_detect_repo
                        ...
                        _repo_keys = list(repos.keys())
                        repo_label = st.selectbox(
                            "Repository (applies to all selected)",
                            _repo_keys,
                            index=max(0, min(default_idx, len(_repo_keys)-1)) if _repo_keys else 0,
                            key=f"mw_batch_repo_{aid}"
                        ) if _repo_keys else None
                    ...
                    with st.expander("📋 File Summary", expanded=True):
                        try:
                            ...
                            if _fp:
                                ...
                                st.markdown(f"_{_s.get('description','')}_")
                                if _s.get("ppdm_hints"):
                                    st.success(
                                        f"**PPDM targets:** "
                                        f"{' · '.join(_s['ppdm_hints'])}"
                                    )
                            ...
                            st.caption(f"Summary unavailable: {_se}")
    ...
    with engine.connect() as conn:
        asgns = conn.execute(text(f"""
            SELECT a.ASSIGNMENT_ID, u.FULL_NAME, a.DUE_DATE,
                   a.STATUS, ISNULL(a.FILE_COUNT,0)
            FROM {at} a
            JOIN {ut} u ON a.user_id = u.USER_ID
...
def _tab_admin(engine, dialect, user, role):
                ...
                with engine.connect() as conn:
                    rows = conn.execute(text(f"""
                        SELECT DISTINCT u.USER_ID, u.FULL_NAME, u.EMAIL,
                               COUNT(gf.file_assignment_id) AS file_count
                        FROM {at} a
                        JOIN dataview.dv_catalog_user u ON a.user_id = u.USER_ID
    ...
    with tab_users:
        ...
        with u_edit:
            if not users:
                ...
                new_email = c2.text_input("Email",     value=su["email"],
                                           key="u_edit_email")
                c3,c4 = st.columns(2)
                roles = ["CATALOGER","DELEGATE","MANAGER"]
                new_role = c3.selectbox("Role", roles,
                                         index=roles.index(su["role"]) if su["role"] in roles else 0,
                                         key="u_edit_role")
        ...
        # ── Impersonate ───────────────────────────────────────────────────────
        with u_impersonate:
            from modules.inv_auth import current_user as _cur_user
                ...
                try:
                    with engine.connect() as con:
                        n_asgn = con.execute(_t(
                            "SELECT COUNT(*) FROM dataview.dv_catalog_assignment "
                            "WHERE user_id=:uid"
                    ...
                    if st.session_state.get("u_del_confirm") == su["user_id"]:
                        ...
                        if c1.button("✅ Yes, delete permanently",
                            ...
                            try:
                                with engine.begin() as con:
                                    con.execute(_t(
                                        "DELETE FROM dataview.dv_catalog_user "
                                        "WHERE user_id=:uid"
                                    ), {"uid": su["user_id"]})
...
def _tab_settings(engine, dialect):
        ...
        with engine.begin() as con:
            con.execute(text("""
                IF NOT EXISTS (
                    ...
                    WHERE s.name='dataview' AND t.name='dv_catalog_setting'
                )
                CREATE TABLE dataview.dv_catalog_setting (
    ...
    def _get(key, default=""):
        try:
            with engine.connect() as con:
                v = con.execute(text(
                    "SELECT SETTING_VALUE FROM dataview.dv_catalog_setting "
                    "WHERE SETTING_KEY=:k"
                ), {"k": key}).scalar()
            return v or default
        except Exception:
    ...
    def _set(key, value, desc=""):
        try:
            with engine.begin() as con:
                con.execute(text("""
                    MERGE dataview.dv_catalog_setting AS tgt
...
def _copy_catalog_files(engine, dialect, dest_root, fmt_filter,
    ...
    for label, sql in queries:
        try:
            ...
            results["details"].append({"File":label,"Status":f"❌ Query: {e}"})
    ...
    for i, row in enumerate(rows):
        ...
        if not src.exists():
            results["missing"] += 1
            results["details"].append({"File": src.name, "Status": "❌ Not found"})
            continue
        ...
        try:
            ...
            results["errors"] += 1
            results["details"].append({"File":src.name,"Status":f"❌ {e}"})
...
def _tab_user_browse(engine):
    import page_file_browser as _fb
...
def _tab_manager_search(engine, dialect):
    ...
    # Type groups covering all cataloged file types
    _TYPE_GROUPS = {
        "All types":      [],
    ...
    q_type = col2.selectbox("Document type", list(_TYPE_GROUPS.keys()),
                             key="mgr_q_type")

    q_status = col3.selectbox(
        "Status",
        ["All", "UNCATALOGED", "CATALOGED", "SKIPPED"],
        key="mgr_q_status"
    ) if "Inventory" in mode else col3.selectbox(
        "Format",
        ["All", "LAS", "DLIS", "LIS", "SEGY", "P190"],
        key="mgr_q_fmt"
    )

    q_ppdm = col4.selectbox(
        "DB loaded",
        ["All", "Yes", "No"],
        key="mgr_q_ppdm"
    ) if "Inventory" in mode else None
    ...
    n_rows = st.slider(
        ...
        value=200, step=50,
        key="mgr_n_rows",
        help="Scroll height scales with row count — 200 rows per 200px"
    )
    display_height = max(300, n_rows)  # ~1px per row

    if st.button("🔍 Search", type="primary", key="mgr_search_btn"):
        try:
            exts = _TYPE_GROUPS.get(q_type, [])
            ...
            if "Inventory" in mode:
                ...
                if q_name:
                    ...
                    params["nm"] = f"%{q_name}%"
                if exts:
                    placeholders = ", ".join(f":ext{i}" for i in range(len(exts)))
                    where.append(f"LOWER(file_ext) IN ({placeholders})")
                ...
                if q_status and q_status != "All":
                    ...
                    params["st"] = q_status
                if q_ppdm == "Yes":
                    where.append("PPDM_LOADED_IND = 'Y'")
                elif q_ppdm == "No":
                    where.append("(PPDM_LOADED_IND = 'N' OR PPDM_LOADED_IND IS NULL)")

                with engine.connect() as con:
                    rows = con.execute(text(
                        f"SELECT TOP {n_rows} "
                        f"FILE_NAME, file_ext, "
                        f"COALESCE(doc_type, FILE_TYPE_GROUP, file_ext) AS TYPE, "
                        f"WELL_NAME, uwi, "
                        f"COALESCE(SUMMARY_DESCRIPTION, '') AS SUMMARY, "
                        f"FILE_SIZE_KB, catalog_status, "
                        f"COALESCE(PPDM_LOADED_IND, 'N') AS PPDM, "
                        f"ROW_CREATED_DATE "
                        f"FROM dataview.dv_global_file_catalog "
                        f"WHERE {' AND '.join(where)} "
                        f"ORDER BY row_created_date DESC"
                    ), params).fetchall()

                df = pd.DataFrame(rows, columns=[
                    "File", "Ext", "Type", "Well", "uwi",
                    "Summary", "Size KB", "Status", "PPDM", "Cataloged"
                ])
                # Format badges
                ...
                df["PPDM"] = df["PPDM"].map(lambda v: "✅" if v == "Y" else "—")
                df["Size KB"] = df["Size KB"].apply(
                    lambda x: f"{float(x):.1f}" if x is not None else "—"
                )

            else:
                # Catalog search — las_catalog tables
                fmt_filter = st.session_state.get("mgr_q_fmt", "All")
                dfs = []
                for tbl, fmt, uwi_col in [
                    ...
                    if fmt_filter not in ("All", fmt):
                        continue
                    if exts and not any(
                        fmt.lower() in e for e in exts
                    ) and q_type != "All types":
                        continue
                    where  = ["1=1"]
                    params = {}
                    if q_name:
                    ...
                    with engine.connect() as con:
                        rows = con.execute(text(
                            f"SELECT TOP {n_rows} file_name, '{fmt}' AS FMT, "
                ...
                if fmt_filter in ("All", "SEGY", "P190") and q_type in ("All types","Seismic"):
                    where  = ["1=1"]
                    params = {}
                    if q_name:
                        where.append("FILE_NAME LIKE :nm")
                    ...
                    with engine.connect() as con:
                        rows = con.execute(text(
                            f"SELECT TOP {n_rows} file_name, FILE_FORMAT, "
            ...
            if df.empty:
                ...
                st.session_state["mgr_search_df"] = df

        except Exception as e:
            st.error(str(e))

    # ── Display results ───────────────────────────────────────────────────────
    df = st.session_state.get("mgr_search_df")
    if df is not None and not df.empty:
        # Ensure all object cols are str to avoid Arrow errors
        for c in df.select_dtypes(include="object").columns:
            df[c] = df[c].astype(str).replace("None","—").replace("nan","—")

        n = len(df)
        c_count, c_slider = st.columns([2, 3])
        c_count.metric("Results", f"{n:,}")
        height = c_slider.slider(
            "Table height (px)", min_value=200, max_value=1200,
            value=min(200 + n * 35, 600), step=100,
            key="mgr_result_height"
        )
    ...
    try:
        ...
        with engine.connect() as con:
            inv_total = con.execute(text(
                "SELECT COUNT(*) FROM dataview.dv_global_file_catalog"
            )).scalar() or 0
        ...
        with engine.connect() as con:
            rows = con.execute(text("""
                SELECT
                    u.FULL_NAME,
                    u.ROLE,
                    COUNT(gf.file_assignment_id)                         AS Assigned,
                ...
                LEFT JOIN dataview.dv_catalog_assignment a
                    ON a.user_id = u.USER_ID
                LEFT JOIN dataview.dv_catalog_group grp
                    ON grp.group_id = a.ASSIGNMENT_ID
                LEFT JOIN dataview.dv_catalog_group_FILE gf
                    ON gf.ASSIGNMENT_ID = a.ASSIGNMENT_ID
                LEFT JOIN dataview.dv_global_file_catalog g
            ...
            with engine.connect() as con:
                rows = con.execute(text(f"""
                    SELECT
                        ...
                        a.STATUS,
                        ISNULL(a.FILE_COUNT,0) AS FILE_COUNT,
                    ...
                    JOIN dataview.dv_catalog_user u ON u.USER_ID = a.user_id
                    LEFT JOIN {ft} gf ON gf.ASSIGNMENT_ID = a.ASSIGNMENT_ID
                    LEFT JOIN {ht} gc ON gc.INVENTORY_ID  = gf.INVENTORY_ID
                    GROUP BY g.GROUP_NAME, g.FILE_TYPE, u.FULL_NAME,
                             a.STATUS, ISNULL(a.FILE_COUNT,0), a.DUE_DATE, a.ASSIGNED_DATE
                ...
                try:
                    ...
                    if not inv_ids:
                        ...
                        with engine.begin() as con:
                            ...
                            con.execute(text(
                                f"INSERT INTO {at} "
                                f"(assignment_id,group_id,user_id,ASSIGNED_BY,"
                            ...
                            for iid in inv_ids:
                                ...
                                con.execute(text(
                                    f"INSERT INTO {ft} "
                                    f"(file_assignment_id,group_id,assignment_id,inventory_id) "
            ...
            with engine.connect() as con:
                asgn_rows = con.execute(text(f"""
                    SELECT a.ASSIGNMENT_ID, g.GROUP_NAME, u.FULL_NAME,
                           COUNT(gf.file_assignment_id) AS files
                    FROM {at} a
                    JOIN {gt} g ON g.group_id = a.group_id
                    JOIN dataview.dv_catalog_user u ON u.USER_ID = a.user_id
                    LEFT JOIN {ft} gf ON gf.ASSIGNMENT_ID = a.ASSIGNMENT_ID
...
def _tab_header_export(engine, dialect):
    ...
    try:
        from modules.file_header_store import ensure_header_tables
    ...
    with tab_well:
        ...
        try:
            with engine.connect() as con:
                n = con.execute(text(
                    ...
                    "FROM dataview.dv_wl_file_catalog"
        ...
        if st.button("📥 Export Well Headers", type="primary",
            ...
            try:
                from modules.file_header_store import export_well_headers
                ...
                if st.button("📋 Stage for Review", type="primary",
                    ...
                    with engine.begin() as con:
                        for _, row in df_up.iterrows():
                            con.execute(text("""
                                INSERT INTO dataview.dv_wl_file_catalog
        ...
        try:
            with engine.connect() as con:
                staged = con.execute(text(
                    "SELECT COUNT(*) FROM dataview.dv_wl_file_catalog "
                    ...
                    for row in rows:
                        try:
                            with engine.begin() as con:
                                ...
                                con.execute(text(
                                    "UPDATE dataview.dv_wl_file_catalog "
    ...
    with tab_seis:
        ...
        try:
            with engine.connect() as con:
                n_s = con.execute(text(
                    "SELECT COUNT(DISTINCT file_name) "
                    "FROM dataview.dv_seis_file_catalog"
        ...
        if st.button("📥 Export Seismic Headers", type="primary",
            ...
            try:
                from modules.file_header_store import export_seis_headers
                ...
                if st.button("📋 Stage Seismic", type="primary",
                    ...
                    with engine.begin() as con:
                        for _, row in df_s.iterrows():
                            con.execute(text("""
                                INSERT INTO dataview.dv_seis_file_catalog
    ...
    try:
        with engine.connect() as con:
            unames = [r[0] for r in con.execute(text(
                "SELECT DISTINCT USER_NAME FROM dataview.dv_catalog_audit "
    ...
    if "office_results" in st.session_state:
        ...
        with col_b:
            ...
            # Show tables for Word
                ...
                st.json({k:v for k,v in kf.items()
                         if not isinstance(v, (list,dict))})

        if s.get("ppdm_hints"):
            st.success(
                f"**Load target:** {', '.join(s['ppdm_hints'])}"
            )
            st.info(
