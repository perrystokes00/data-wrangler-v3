...
import exporters   # multi-format result-set exporters (sits beside this file)
...
def run(engine=None):
    ...
    if st.session_state.get("wm_docs_page"):
        ...
        if st.button("← Back to map", key="docs_back"):
            ...
            st.rerun()
        try:
            import page_selected_documents
...
# ── Resolve query filter for SQL push-down ─────────────────────
    ...
    with mapcol:
        ...
        if _geo_on or "geo_wellpts" in active_db:
            try:
                from geography_layers import add_geography_layer, add_well_points
                for _ak in _geo_on:
