...
_OP_RE = _re.compile(
    r"[A-Z][A-Za-z0-9&, .'-]{3,35}"
    r"(LLC|INC|LTD|CORP|LP|COMPANY|CO|OPERATING|ENERGY|OIL|PETROLEUM|RESOURCES)",
    _re.IGNORECASE
)


def _extract_operator(line: str) -> str:
    candidate = _clean(line[71:103])
    if candidate and candidate[0].isalpha() and len(candidate) > 4:
        return candidate[:80]
    m = _OP_RE.search(line[16:160])
    if m:
        return _clean(m.group(0))[:80]
    return candidate[:80]
...
def parse_maf016(
    ...
    with open(filepath, encoding="latin-1", errors="replace") as f:
        for line in f:
            ...
            operator = _extract_operator(line)
            ...
            spud     = _parse_date(dates[0]) if dates else None
            compl    = _parse_date(dates[1]) if len(dates) > 1 else None
            type_s   = line[238:240].strip() if len(line) >= 240 else ""
            wells.append({
                "uwi":            uwi,
                "well_name":      lease_nm[:80] or "UNKNOWN",
                "well_type":      WELL_TYPE.get(type_s,"OIL"),
                "well_status":    WELL_STATUS.get(rec_type,"ACTIVE"),
                "province_state": "TX",
                "country":        "US",
...
def load_wells(wells: list[dict], dry_run: bool = False) -> int:
    ...
    - If UWI exists: UPDATE well_status, TD, dates (not coordinates)
    ...
    # Deduplicate parsed batch by UWI (keep last occurrence)
    seen = {}
    for w in wells:
        seen[w["uwi"]] = w
    wells = list(seen.values())
    print(f"After dedup: {len(wells):,} unique UWIs")
            ...
            for i in range(0, len(rows), chunk):
                ...
                for r in batch:
                    try:
                        con.execute(text("""
                            IF NOT EXISTS (
                                SELECT 1 FROM dataview.dv_well WHERE uwi = :uwi
                            )
                            INSERT INTO dataview.dv_well (
                                uwi, well_name, well_type, well_status,
                                province_state, country, county,
                                final_td, depth_datum, spud_date, completion_date,
                                api_num, active_ind, source,
