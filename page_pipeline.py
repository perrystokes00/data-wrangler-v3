...
def render(S):
        ...
        if db_type == "SQL Server":
            ...
            with col_l:
                server   = st.text_input("Server / Instance",
                                         value="PERRY\\SQLEXPRESS",
                                         placeholder="localhost\\SQLEXPRESS  or  192.168.1.10,1433")
                database = st.text_input("Database", value="PPDM39_DEMO_1")
            ...
            if st.button("🔌 Connect", use_container_width=True):
                ...
                if result.ok:
                    ...
                    try:
                        ...
                        _cpath = r"schema_registry\ppdm_39_schema_domain.json"
        ...
        if getattr(S, "_post_connect_choice", False) and S.engine:
            ...
            with st.expander("🗄️ Initialize / Migrate Database", expanded=False):
                st.caption(
                    ...
                    "PPDM 3.9 database. Safe to run on existing databases — "
            ...
            if uploaded:
                ...
                if not ingest.ok:
                    ...
                    if st.button(f"📥 Load to `{ingest.staging_name}` ({ingest.col_count} columns)",
                        ...
                        with st.spinner("Loading to staging via BULK INSERT..."):
                            if S.demo:
                        ...
                        if sr.ok:
                            ...
                            # Drop any None/empty column names from trailing empty cols
                            ...
                            # (before promote so retries of same session don't re-flag)
                            if not S.demo and S.staging_df is not None and not getattr(S, "_stg_file_recorded", False):
                                ...
                                if _is_dup:
                                    st.warning(
                                        f"⚠️ This data was previously loaded into "
                                        f"`{S.target_table}` on {_dup_ts}. "
                                        f"SHA-256: `{_hash[:16]}…`  "
                                        f"Continuing — use Promote → Merge if updating existing rows."
                                    )
    ...
    # ═══════════════════════════════════════════════════════════════════════
        ...
        shdr(
            "Stage 4 · Select Target PPDM Table",
            "Upload your PPDM 3.9 JSON catalog, then choose the target table for this load.",
        )

        # ── 4a. Schema status ───────────────────────────────────────────
        if S.ppdm_schema is None:
            st.warning(
                f"No schema loaded. Select a schema from the sidebar "
                f"(currently: **{S.schema_variant}**)."
            )
            st.stop()
        else:
                    ...
                    with _tab_cols:
                        if S.engine and not S.demo:
                            _df_c = _run_tab_query("""
                                ...
                                WHERE s.name='dbo' AND t.name=:tbl ORDER BY c.column_id""", selected, S.engine)
                        else:
                    ...
                    with _tab_fko:
                        if S.engine and not S.demo:
                            _df_fo = _run_tab_query("""
                                ...
                                WHERE cs.name='dbo' AND ct.name=:tbl
                    ...
                    with _tab_fki:
                        if S.engine and not S.demo:
                            _df_fi = _run_tab_query("""
                                ...
                                JOIN sys.schemas ps ON ps.schema_id=pt.schema_id
                                JOIN sys.columns pc ON pc.object_id=fk.referenced_object_id AND pc.column_id=fkc.referenced_column_id
                                WHERE ps.name='dbo' AND pt.name=:tbl
                            ...
                            try:
                                ...
                                with S.engine.connect() as _ec:
                                    _db_rows = _ec.execute(_t(
                                        ...
                                        "WHERE LOWER(t.name)=LOWER(:tbl) AND s.name='dbo' "
                                ...
                                for _r in _db_rows:
                                    if _r[0].upper() not in _json_names:
                                        _extra.append(_CD(
                                            ...
                                            fk_table_schema= "dbo" if _r[5] else None,
                                            fk_table_name  = _r[6],
                                            fk_column_name = None,
                                            check_constraints = [],
                                        ))
                ...
                try:
                    ...
                    for tname in entity_order:
                        tschema = next((c.parent_schema for c in entity_constraints
                                       if c.parent_table.upper() == tname.upper()), "dbo")
                      ...
                      try:
                        ...
                        if _fk_dialect == "oracle":
                            ...
                            _q = (
                                ...
                                " JOIN sys.schemas ps ON ps.schema_id=pt.schema_id"
                                " JOIN sys.columns pc ON pc.object_id=fk.referenced_object_id AND pc.column_id=fkc.referenced_column_id"
                                " WHERE cs.name='dbo' AND ct.name=:tbl AND pt.name<>:tbl"
                                ...
                                "     AND ct2.name=pt.name AND cs2.name='dbo' AND ct.name<>:tbl)),"
                                                ...
                                                if _fk_dialect == "oracle":
                                                    ...
                                                    if _pt_rc == 0:
                                                        ...
                                                        _mv = [r[0] for r in _ec3.execute(_stext(
                                                            ...
                                                            f"AND NOT EXISTS (SELECT 1 FROM [dbo].[{_pt}] ref WHERE ref.[{_pk_col}]=src.[{_src_col3}])"
                                                        )).fetchall()]
                                            if _mv:
    ...
    # ═══════════════════════════════════════════════════════════════════════
        ...
        shdr("Stage 8 · Promote to Target Table",
             "Review the SQL and confirm to execute the INSERT into the PPDM target table.")
        ...
        if not S.promoted:
            if df is None or not S.col_mapping or not S.val_report:
                ...
                sql_preview = (
                    f"INSERT INTO [dbo].[{S.target_table}]\n"
                    f"  ({tgt_cols_sql})\n"
                    f"SELECT\n"
                    f"  {src_cols_sql}\n"
                ...
                if _do_merge and _merge_pk_cols:
                    ...
                    sql_preview = (
                        f"MERGE [dbo].[{S.target_table}] AS tgt\n"
                ...
                with st.expander("📊 Load Reconciliation", expanded=False):
                    try:
                        ...
                        if S.engine and _mp and _stg_tbl:
                            ...
                            # Target counts — query PPDM table
                            ...
                            with S.engine.connect() as _rc:
                                ...
                                _tgt_row = _rc.execute(_rt(
                                    f"SELECT {_tgt_count_sql} FROM [dbo].[{S.target_table}]"
                                )).fetchone()
                            ...
                            if _r_rows:
                                ...
                                st.dataframe(
                                    ...
                                    column_config={
                                        ...
                                        "PPDM Column":   st.column_config.TextColumn(width="medium"),
            ...
            with col2:
                if not S.demo:
                    st.markdown(f"**Target table:** `dbo.{S.target_table}`")
