...
try:
    from catalog_docs import list_documents, _open_native
except Exception:                       # pragma: no cover
    try:
        from modules.catalog_docs import list_documents, _open_native
    except Exception:
...
try:
    import file_viewer as _file_viewer
except Exception:
    try:
...
def _doc_view_page(engine, inv):
    ...
    # ── Close button at the END of the view (so you don't scroll back up) ──
    st.divider()
    if st.button("✖ Close document", key="seldoc_close_bottom",
