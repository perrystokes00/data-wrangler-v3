...
def _run_scan(engine, dialect, root: str, exts: set):
    ...
    from sqlalchemy import text as _t
    from fingerprint import file_fingerprint, DEDUPE_SQL   # single source of truth
...
def _tab_browse(engine, dialect):
    ...
    if st.button("🗂 Refresh cataloged status", key="wb_backfill_cat",
        ...
        try:
            try:
                from catalog_readiness import reconcile_readiness
            except Exception:
                from modules.catalog_readiness import reconcile_readiness
...
def _do_extract(fpath: str, fext: str) -> tuple:
    ...
    try:
        if fext == ".pdf":
            ...
            try:
                from modules.pdf_survey_catalog import extract_core, RT_CORE
            except ImportError:
            ...
            from segy_header import sample_trace_rows
        ...
        if fext == ".pdf":
            ...
            if uwi and rt != RT_SCOUT:
                try:
                    ...
                    try:
                        from modules.catalog_capture import capture as _cap
                    except ImportError:
                        from catalog_capture import capture as _cap
            ...
            try:
                from modules.witsml_catalog import load_witsml as _witsml
            except ImportError:
                from witsml_catalog import load_witsml as _witsml
            ...
            try:
                from modules.json_well_log_catalog import load_json_well_log as _jwl
            except ImportError:
                from json_well_log_catalog import load_json_well_log as _jwl
            ...
            try:
                from modules.dv_office_loader import dispatch as _office
            except ImportError:
                from dv_office_loader import dispatch as _office
...
def _pipeline_stages(engine, dialect):
    ...
    with st.expander(
        ...
        if c2.button("Run enrichment", type="primary", key="pl_en_run",
                     use_container_width=True):
            import enrich_file_headers as _en
    ...
    with st.expander(
        ...
        if d4.button("Run collect", type="primary", key="pl_d_run",
                     use_container_width=True):
            import collect_final_documents as _cf
            a = _types.SimpleNamespace(
                server=_host, database=_db, odbc_driver="",
    ...
    with st.expander(
        ...
        if sc2.button("Generate scorecard", type="primary", key="pl_sc_run",
                      use_container_width=True):
            import catalog_scorecard as _sc
            raw = engine.raw_connection()
            data, err = None, None
            try:
