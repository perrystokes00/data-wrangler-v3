...
def _detect_delimiter(sample: str) -> str:
    ...
    # Prefer | and \t over comma since commas appear in data values
    for d in ('|', '\t', ';', ','):
        if counts[d] > 0:
            return d
    try:
...
def _load_to_staging_sqlserver(engine, ingest, schema, table, columns) -> StagingResult:
    """SQL Server staging via BULK INSERT."""
    from sqlalchemy import text

    csv_path = ingest.csv_path.replace("'", "''")

    _using_bulk_dir = ingest.csv_path.upper().startswith("C:\\BULK") if ingest.csv_path else False
    if ingest.csv_path and not _using_bulk_dir:
        import warnings as _w
        _w.warn(
            "Staging CSV is in temp dir. "
            "Ensure the SQL Server service account has READ access, "
            "or grant access to C:\\Bulk for reliable BULK INSERT."
        )
    ...
    _bulk_delim = ingest.delimiter if ingest.delimiter else ","
    _err_path   = csv_path.replace(".csv", "_errors.txt")
    sql_bulk    = (f"BULK INSERT [{schema}].[{table}]\nFROM '{csv_path}'\n"
                   f"WITH (FIRSTROW=2, FIELDTERMINATOR='{_bulk_delim}', "
                   f"ROWTERMINATOR='0x0D0A', CODEPAGE='65001', TABLOCK, "
                   f"MAXERRORS=0)")
    ...
    # Pre-flight: verify CSV has data rows before BULK INSERT
    try:
        with open(ingest.csv_path, encoding="utf-8") as _pf:
            _line_count = sum(1 for _ in _pf) - 1  # subtract header row
        if _line_count <= 0:
            return StagingResult(ok=False,
                                 message="Staging CSV is empty — no data rows found. "
                                         "Check the sheet selection and header row setting.",
                                 table_name=f"{schema}.{table}")
    except Exception:
        pass  # if we can't read it, let BULK INSERT try anyway
    ...
    try:
        ...
        ingest.row_count = n
        # Only delete temp file — never delete the original source file
        _is_temp = (
            ingest.csv_path and
            ingest.csv_path.upper().startswith("C:\\BULK\\STAGE_")
        )
        if _is_temp:
            try:
        ...
        return StagingResult(
            ok=True,
            message=f"Loaded {n:,} rows × {len(columns)} cols into [{schema}].[{table}]",
            rows_loaded=n,
            table_name=f"{schema}.{table}",
        )
    except Exception as exc:
