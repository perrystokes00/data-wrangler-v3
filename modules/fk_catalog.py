"""
fk_catalog.py  --  DataView: FK Catalog Reader
Loads the pre-built FK catalog JSON and provides fast dictionary-based
lookups as drop-in replacements for live DB introspection calls in fk.py.
...
def _catalog_path(dialect: str) -> pathlib.Path:
    return _BASE / "dataview_fk_catalog.json"

def _domain_path(dialect: str) -> pathlib.Path:
    return _BASE / "dataview_schema_domain.json"
...
class FKCatalog:
    ...
    def introspect(self, table_name: str, schema: str = "dataview"):
        """
...
def get_catalog(engine=None, dialect: str = "") -> FKCatalog:
    ...
    Falls back to sqlserver if neither is provided.
    """
    if not dialect and engine is not None:
        try:
            ...
            dialect = "sqlserver"
    dialect = (dialect or "sqlserver").lower()
