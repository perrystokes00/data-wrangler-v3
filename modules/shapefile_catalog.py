        ...
        try:
            ...
            with engine.begin() as con:
                ...
                con.execute(text(f"""
                    ...
                    (UWI, WELL_NAME, OPERATOR, SURFACE_LATITUDE,
                     SURFACE_LONGITUDE, PROVINCE_STATE, COUNTRY_NAME,
                     SOURCE, ROW_CREATED_BY, ROW_CREATED_DATE)
                    VALUES
                    (:uwi, :wn, :op, :lat, :lon, :st, :ctry,
                     :src, :by, {now})
                """), {
                    "uwi":  uwi[:40],
                    ...
                    "st":   state[:40]   if state   else None,
                    "ctry": country[:40] if country else "US",
                    "src":  source[:40],
                    "by":   "DataWrangler",
                })
            result["loaded"] += 1
