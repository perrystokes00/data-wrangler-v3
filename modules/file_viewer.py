...
Usage:
    from modules.file_viewer import view
...
def _view_pdf(file_path: str):
    ...
    try:
        ...
        h   = st.slider("Height (px)", 400, 1200, 700, 50,
                        key=f"pdfh_{file_path}")
        st.markdown(
...
def _view_las(file_path: str):
    ...
    with st.expander(f"📈 Curves ({len(curve_items)})", expanded=True):
...
def _view_dlis(file_path: str):
    ...
    try:
        ...
        ch_info = [{"Name": ch.name, "Unit": str(ch.units),
                   ...
                   for ch in channels]
        with st.expander(f"📡 Channels ({len(channels)})", expanded=True):
...
def _view_segy(file_path: str):
    ...
    try:
        with segyio.open(file_path, ignore_geometry=True) as f:
            ...
            with st.expander("🗂️ Binary header", expanded=True):
                st.dataframe(
            ...
            with st.expander(
                    f"📋 Trace headers (first {n_preview})", expanded=False):
...
def _view_p190(file_path: str):
    import pandas as pd
    try:
        from modules.p190_catalog import parse_p190
...
def _view_shapefile(file_path: str):
    ...
    with st.expander("📋 Attribute table", expanded=True):
...
def _view_word(file_path: str):
    ...
    if ext == ".docx":
        ...
        # Tables
        if doc.tables:
            with st.expander(f"📊 Tables ({len(doc.tables)})", expanded=False):
