    ...
    try:
        ...
        try:
            with engine.connect() as _dtc:
                _dt_rows = _dtc.execute(_txt(
                    ...
                    "WHERE LOWER(tbl.name) = LOWER(:tbl) AND s.name = 'dbo' "
        ...
        try:
            with engine.connect() as _pk_con:
                _pk_rows = _pk_con.execute(_txt("""
                    ...
                    JOIN sys.schemas s ON s.schema_id = t.schema_id
                    WHERE i.is_primary_key = 1 AND LOWER(t.name) = LOWER(:tbl) AND s.name = 'dbo'
        ...
        try:
            ...
            _fk_rows = engine.connect().execute(_txt("""
                ...
                WHERE LOWER(ct.name) = LOWER(:tbl) AND s.name = 'dbo'
            """), {"tbl": target_table}).fetchall()
            ...
            for parent_tbl, col_pairs in _fk_groups.items():
                ...
                try:
                    with engine.connect() as _fk_chk_con:
                        _par_count = _fk_chk_con.execute(_txt(
                            f"SELECT COUNT(*) FROM [dbo].[{parent_tbl}] WITH (NOLOCK)"
                        )).scalar() or 0
                    if _par_count == 0:
                ...
                if _skip_fk or not _join_parts:
                    continue
                _fk_filters.append(
                    f"EXISTS (SELECT 1 FROM [dbo].[{parent_tbl}] fk_{parent_tbl} "
