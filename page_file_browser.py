...
def render(engine=None):
    ...
    st.caption(
        ...
        "Preview headers and plots. Copy files or export PPDM data."
    )
    ...
    tab_catalog, tab_inventory, tab_wells, tab_seis, tab_ppdm = st.tabs([
        ...
        "🛢 Well Search", "🌊 Seismic Search", "📊 PPDM Export"
    ])
...
def _render_ppdm_export(engine):
    """Export PPDM table data to CSV or Excel."""
    import pandas as pd
    from sqlalchemy import text

    st.markdown("#### 🛢 Export PPDM Data")
    st.caption("Export well and seismic data from the PPDM schema to CSV or Excel.")

    PPDM_TABLES = {
        "WELL":       ("dbo.WELL",       "UWI, WELL_NAME, OPERATOR, FIELD_NAME, "
        ...
        "SEIS_SET":   ("dbo.SEIS_SET",   "SEIS_SET_ID, SEIS_SET_NAME, SEIS_SET_TYPE, "
...
# ─────────────────────────────────────────────────────────────────────────────
# Well search — PPDM dbo.WELL with file preview
# ─────────────────────────────────────────────────────────────────────────────

def _render_well_search(engine):
    """
    Search well files across three scenarios:
    1. Cataloged + PPDM matched  — LAS/DLIS/LIS with UWI in dbo.WELL
    2. Cataloged + unmatched UWI — cataloged but UWI not in dbo.WELL
    ...
    st.caption(
        ...
        "Shows the UWI stored in the file header and whether it exists in PPDM."
    )

    # ── Filters ───────────────────────────────────────────────────────────────
    c1,c2,c3,c4 = st.columns(4)
    q_uwi   = c1.text_input("UWI (file or PPDM)", key="ws_uwi",
                              placeholder="e.g. 42-001-00001")
    q_well  = c2.text_input("Well name",          key="ws_well",
                              placeholder="e.g. ANADARKO")
    ...
    q_scope = c4.selectbox("Show",
                            ...
                            key="ws_scope")
    c5,c6   = st.columns([1,3])
    max_r   = c5.number_input("Max results", 10, 5000, 200, key="ws_max")
    search  = c6.button("🔍 Search", type="primary", key="ws_btn")
    ...
    if search or "ws_results" in st.session_state:
        if search:
            try:
                        ...
                        with engine.connect() as con:
                            r = con.execute(text(f"""
                                SELECT TOP {int(max_r)}
                                    ...
                                    END              AS MATCH_STATUS
                                FROM {tbl} f
                                LEFT JOIN dbo.WELL w ON w.UWI = f.UWI
                ...
                df = pd.DataFrame(rows_all, columns=[
                    ...
                    "Size KB","Date","PPDM UWI","PPDM Well Name","Status"
        ...
        m2.metric("✅ PPDM matched", len(df[df["Status"]=="✅ Matched"]))
                ...
                for i, (_, row) in enumerate(sel_w.iterrows()):
                    ...
                    try:
                        if fmt == "LAS":
                            ...
                            from modules.dlis_catalog import catalog_dlis_file as _cf
                            r = _cf(engine, fp, repo_id, uwi=uwi)
                        elif fmt == "LIS":
                            from modules.dlis_catalog import catalog_lis_file as _cf
                            r = _cf(engine, fp, repo_id, uwi=uwi)
                        else:
    ...
    c1,c2,c3,c4 = st.columns(4)
    q_uwi  = c1.text_input("UWI",       key="ws_uwi",
                             placeholder="e.g. 42-001-00001")
    q_well = c2.text_input("Well name", key="ws_well",
                             placeholder="e.g. ANADARKO")
    ...
    c5,c6    = st.columns([1,3])
    max_rows = c5.number_input("Max results", 10, 2000, 100, key="ws_max")
    search   = c6.button("🔍 Search Wells", type="primary", key="ws_btn")
                ...
                try:
                    ...
                    with engine.connect() as con:
                        rows = con.execute(text(
                            ...
                            f"FROM dbo.WELL WHERE UWI IS NOT NULL "
...
# ─────────────────────────────────────────────────────────────────────────────
# Seismic search — PPDM dbo.SEIS_SET / SEIS_FILE_CATALOG with preview
# ─────────────────────────────────────────────────────────────────────────────

def _render_seis_search(engine):
    """
    Search seismic files across three scenarios:
    1. Cataloged + PPDM matched  — SEIS_FILE_CATALOG with SEIS_SET match
    ...
    # ── Filters ───────────────────────────────────────────────────────────────
    c1,c2,c3,c4 = st.columns(4)
    q_sv   = c1.text_input("Survey / file name", key="ss_sv",
                             placeholder="e.g. CENTRAL_AUSTRALIA")
    q_fmt  = c2.selectbox("Format", ["All","SEGY","P190"], key="ss_fmt")
    q_scope = c3.selectbox("Show",
                            ["All files","Matched only",
                             "Unmatched","Inventory only"],
                            key="ss_scope")
    max_r  = c4.number_input("Max results", 10, 5000, 200, key="ss_max")
    search = st.button("🔍 Search", type="primary", key="ss_btn")
    ...
    if search or "ss_results" in st.session_state:
        if search:
            ...
            if rows_all:
                df = pd.DataFrame(rows_all, columns=[
                    ...
                    "Size KB","Date","SEIS_SET_ID","PPDM Survey",
        ...
        display = df[["Status","Format","File Name","Catalog Survey",
                      "PPDM Survey","Detected Survey","Detected Line",
        ...
        repo_label = c_repo.selectbox("Repository for batch",
                                       ...
                                       key="ss_cat_repo")
        repo_id    = repo_opts[repo_label]
        seed_ppdm  = c_seed.checkbox("Seed PPDM", value=True,
                                      key="ss_cat_seed")
                ...
                for i, (_, row) in enumerate(sel.iterrows()):
                    ...
                    try:
                        ...
                        if r.get("ok"):
                            ...
                            # Store headers
                            try:
                                from modules.file_header_store import (
        ...
        st.divider()
        st.markdown("#### 📂 Copy to folder")
        dest = st.text_input("Destination", key="ss_dest",
    ...
    c1,c2,c3 = st.columns(3)
    q_sv  = c1.text_input("Survey name", key="ss_sv",
                            placeholder="e.g. CENTRAL_AUSTRALIA")
    q_fmt = c2.selectbox("Format", ["All","SEGY","SEG-Y","P190"],
                          key="ss_fmt")
    max_r = c3.number_input("Max results", 10, 2000, 100, key="ss_max")
    search = st.button("🔍 Search", type="primary", key="ss_btn")
