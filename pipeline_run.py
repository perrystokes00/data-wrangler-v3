...
from fingerprint import file_fingerprint, DEDUPE_SQL   # noqa: E402
...
def default_exts():
    ...
    """
    try:
        import page_workbench as _wb
        opt_in = set(_wb.CSV_EXTS)
    except Exception:
        opt_in = {".csv", ".tsv"}
    try:
        from modules.file_summarizer import SUPPORTED_EXTS
        base = set(SUPPORTED_EXTS)
    except Exception:
        try:
            from file_summarizer import SUPPORTED_EXTS
            base = set(SUPPORTED_EXTS)
        except Exception:
...
def _stage_scan(engine, root, exts, log):
    ...
    import page_workbench as wb                      # for EXT_GROUP
...
def _capture_pool_init(url):
    ...
    try:
        ...
        import worker_core  # noqa: F401 — warm the import once per worker
...
def _stage_extract_capture(engine, workers, log, exts=None, do_capture=True):
    ...
    import page_workbench as wb
    try:
        from page_workbench import SELF_PARSING_EXTS
...
def _stage_capture(engine, dialect, log, exts=None, workers=8, parallel=False):
    ...
    try:
        from page_workbench import (_do_extract, _load_rows_to_catalog,
                                     SELF_PARSING_EXTS)
    except Exception as e:
        log(f"[capture] skipped — page_workbench helpers unavailable: {e}")
        return {"capture_files": 0, "capture_rows": 0, "capture_ok": 0}
    try:
        from catalog_capture import reset_replace_state
    except Exception:
        try:
            from modules.catalog_capture import reset_replace_state
        except Exception:
...
def _stage_enrich(engine, ref, apply, log, report_dir=None):
    ...
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    import enrich_file_headers as en
