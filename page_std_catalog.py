...
def run():
    ...
    if st.button("← File Catalog", key="wl_back_to_landing"):
        ...
        st.rerun()
    st.title("🛢️ Well Log Catalog")
    st.caption("Index and search LAS, DLIS and LIS files — matched to PPDM wells.")
    ...
    tab_dash, tab_search, tab_catalog = st.tabs([
...
def run_seismic():
    ...
    if st.button("← File Catalog", key="seis_back_to_landing"):
        ...
        st.session_state.app_mode = "file_catalog"
        st.rerun()
    st.title("🌊 Seismic Catalog")
    st.caption("Index and search SEG-Y and P1/90 files — bounding boxes, survey maps, EBCDIC headers.")
...
def _render_repositories(engine):
    ...
    sub_show, sub_add, sub_edit, sub_delete, sub_clear = st.tabs(["📋 Show", "➕ Add", "✏️ Edit", "🗑 Delete", "🔄 Clear & Recatalog"])
    ...
    with sub_show:
        if repos.empty:
            ...
            sel_repo = st.selectbox("Select repository", repos["REPOSITORY_NAME"].tolist(),
                                     key="repo_show_sel")
            sel_row  = repos[repos["REPOSITORY_NAME"] == sel_repo].iloc[0]
            sel_id   = sel_row["REPOSITORY_ID"]
            sel_path = sel_row["BASE_PATH"]
            st.caption(f"`{sel_path}`")

            tables = {
                "LAS":   ("LAS_FILE",          ["UWI","WELL_NAME","FILE_NAME","FILE_SIZE_KB","CATALOG_DATE"]),
                "DLIS":  ("DLIS_FILE",         ["UWI","FILE_NAME","FILE_SIZE_KB","CATALOG_DATE"]),
                "LIS":   ("LIS_FILE",          ["UWI","FILE_NAME","FILE_SIZE_KB","CATALOG_DATE"]),
                "SEG-Y / P190": ("SEIS_FILE_CATALOG", ["FILE_FORMAT","SURVEY_NAME","FILE_NAME","FILE_SIZE_KB","CATALOG_DATE"]),
            }

            for fmt, (tbl, cols) in tables.items():
                try:
                    ...
                    if rows:
                        ...
                        with st.expander(f"**{fmt}** — {len(rows):,} file(s)", expanded=True):
    ...
    with sub_clear:
        if repos.empty:
            st.info("No repositories to clear."); return
        from sqlalchemy import text as _text

        if msg := st.session_state.pop("repo_clr_msg", None):
            st.success(msg)

        clr_sel = st.selectbox("Select repository", repos["REPOSITORY_NAME"].tolist(),
                                key="repo_clr_sel")
        clr_row = repos[repos["REPOSITORY_NAME"] == clr_sel].iloc[0]
        clr_id  = clr_row["REPOSITORY_ID"]
        clr_path = clr_row["BASE_PATH"]

        # Count existing entries
        try:
            with engine.connect() as con:
                counts = {
                    "LAS":  con.execute(_text("SELECT COUNT(*) FROM [las_catalog].[LAS_FILE]  WHERE REPOSITORY_ID=:id"),{"id":clr_id}).scalar() or 0,
                    "DLIS": con.execute(_text("SELECT COUNT(*) FROM [las_catalog].[DLIS_FILE] WHERE REPOSITORY_ID=:id"),{"id":clr_id}).scalar() or 0,
                    "LIS":  con.execute(_text("SELECT COUNT(*) FROM [las_catalog].[LIS_FILE]  WHERE REPOSITORY_ID=:id"),{"id":clr_id}).scalar() or 0,
                    "SEIS": con.execute(_text("SELECT COUNT(*) FROM [las_catalog].[SEIS_FILE_CATALOG] WHERE REPOSITORY_ID=:id"),{"id":clr_id}).scalar() or 0,
                }
        except Exception as e:
            st.error(str(e)); return
        ...
        st.info(
            f"**{clr_sel}** — `{clr_path}`\n\n"
            f"LAS: {counts['LAS']:,} · DLIS: {counts['DLIS']:,} · "
            f"LIS: {counts['LIS']:,} · Seismic: {counts['SEIS']:,} · "
            f"**Total: {total:,}**"
        )

        st.warning("⚠️ This will delete all catalog entries for this repository.")

        if st.button("🗑 Clear catalog entries", type="primary",
                     key="repo_clr_btn"):
            try:
                with engine.begin() as con:
                    # Delete in FK-safe order (children before parents)
                    _sub = "(SELECT DLIS_FILE_ID FROM [las_catalog].[DLIS_FILE] WHERE REPOSITORY_ID=:id)"
                    for tbl in ["DLIS_CHANNEL","DLIS_FRAME","DLIS_PARAMETER","DLIS_LOGICAL_FILE"]:
                        con.execute(_text(
                            f"DELETE FROM [las_catalog].[{tbl}] WHERE DLIS_FILE_ID IN {_sub}"
                        ), {"id": clr_id})
                    _lis_sub = "(SELECT LIS_FILE_ID FROM [las_catalog].[LIS_FILE] WHERE REPOSITORY_ID=:id)"
                    con.execute(_text(
                        f"DELETE FROM [las_catalog].[LIS_CHANNEL] WHERE LIS_FILE_ID IN {_lis_sub}"
                    ), {"id": clr_id})
                    _las_sub = "(SELECT LAS_FILE_ID FROM [las_catalog].[LAS_FILE] WHERE REPOSITORY_ID=:id)"
                    for tbl in ["LAS_FILE_CURVE","LAS_FILE_PARAMETER"]:
                        con.execute(_text(
                            f"DELETE FROM [las_catalog].[{tbl}] WHERE LAS_FILE_ID IN {_las_sub}"
                        ), {"id": clr_id})
                    # Now delete parent tables
                    for tbl in ["LAS_FILE","DLIS_FILE","LIS_FILE","SEIS_FILE_CATALOG"]:
                        con.execute(_text(
                            f"DELETE FROM [las_catalog].[{tbl}] WHERE REPOSITORY_ID=:id"
                        ), {"id": clr_id})
                st.session_state["repo_clr_msg"] = f"✅ Cleared {total:,} entries from **{clr_sel}** — go to the Catalog tab to re-catalog."
                st.rerun()
            except Exception as e:
                st.error(str(e))
...
def _render_catalog_las_directory(engine, repository_id, base_path, source):
    _show_catalog_btn(engine, "LAS", "LAS_FILE", "LAS_FILE_ID",
                      extra_cols=["UWI","WELL_NAME","FILE_NAME"], key_suffix="lasdir")
    st.divider()
...
def _render_catalog_dlis_directory(engine, repository_id, base_path, source):
    _show_catalog_btn(engine, "DLIS", "DLIS_FILE", "DLIS_FILE_ID",
                      extra_cols=["UWI","FILE_NAME"], key_suffix="dlisdir")
    st.divider()
    st.info(
...
def _render_catalog_lis_directory(engine, repository_id, base_path, source):
    _show_catalog_btn(engine, "LIS", "LIS_FILE", "LIS_FILE_ID",
                      extra_cols=["UWI","FILE_NAME"], key_suffix="lisdir")
    st.divider()
    st.info(
...
def _render_review(engine):
    ...
    st.caption("The correct catalog (LAS, DLIS, LIS, SEG-Y, P190) is automatically determined from each file's extension.")
    ...
    with col1:
        fmt_filter = st.selectbox(
            "Format", ["All", "LAS", "DLIS", "LIS", "SEGY", "P190"], key="map_rev_fmt"
        )
    with col2:
    ...
    if "FILE_FORMAT" in df.columns:
        fmt_counts = df["FILE_FORMAT"].value_counts()
        st.caption("Formats: " + " · ".join(
            f"**{fmt}**: {n:,}" for fmt, n in fmt_counts.items()
        ))
...
def _render_path_repair(engine):
    """Find catalog records with filename-only paths and repair them by scanning directories."""
    from sqlalchemy import text
    import os

    st.markdown("#### 🔧 Path Repair")
    st.caption(
        "Find cataloged files stored with filename-only paths (no directory). "
        "Scan a directory to auto-match and repair them."
    )

    # Count broken paths in each table
    tables = {
        "SEG-Y / P190": ("las_catalog.SEIS_FILE_CATALOG", "SEIS_FILE_ID", "FILE_NAME"),
        "LAS":          ("las_catalog.LAS_FILE",          "LAS_FILE_ID",  "FILE_NAME"),
        "DLIS":         ("las_catalog.DLIS_FILE",         "DLIS_FILE_ID", "FILE_NAME"),
        "LIS":          ("las_catalog.LIS_FILE",          "LIS_FILE_ID",  "FILE_NAME"),
    }

    broken = {}
    for label, (tbl, pk, fn_col) in tables.items():
        try:
            with engine.connect() as con:
                n = con.execute(text(
                    f"SELECT COUNT(*) FROM [{tbl.split('.')[0]}].[{tbl.split('.')[1]}] "
                    f"WHERE [{fn_col}] NOT LIKE '%\\%' "
                    f"AND [{fn_col}] NOT LIKE '%/%'"
                )).scalar()
            if n:
                broken[label] = (tbl, pk, fn_col, n)
        except Exception:
            pass

    if not broken:
        st.success("✅ All cataloged file paths look complete — no repairs needed.")
        return

    for label, (tbl, pk, fn_col, n) in broken.items():
        st.warning(f"**{label}**: {n} record(s) with filename-only paths")

    st.divider()
    scan_dir = st.text_input(
        "Directory to scan for matching files",
        key="path_repair_dir",
        placeholder=r"e.g. C:\Bulk\Seismic"
    )
    recursive = st.checkbox("Scan subdirectories", value=True, key="path_repair_recursive")

    if st.button("🔍 Scan & Repair", type="primary", key="path_repair_btn",
                 disabled=not scan_dir.strip()):
        if not os.path.isdir(scan_dir.strip()):
            st.error(f"Directory not found: {scan_dir.strip()}")
            return

        with st.spinner("Scanning directory…"):
            # Build filename → full path index
            path_index = {}
            walker = os.walk(scan_dir.strip()) if recursive else [
                (scan_dir.strip(), [], os.listdir(scan_dir.strip()))
            ]
            for dirpath, _, files in walker:
                for fname in files:
                    path_index[fname.lower()] = os.path.join(dirpath, fname)

        st.caption(f"Found {len(path_index):,} files in directory.")
        ...
        for label, (tbl, pk, fn_col, n) in broken.items():
            ...
            try:
                with engine.connect() as con:
                    rows = con.execute(text(
                        f"SELECT [{pk}], [{fn_col}] FROM [{schema}].[{table}] "
                        f"WHERE [{fn_col}] NOT LIKE '%\\\\%' "
                        f"AND [{fn_col}] NOT LIKE '%/%'"
                    )).fetchall()
                ...
                with engine.begin() as con:
                    for row in rows:
                        ...
                        full = path_index.get(str(fname).lower())
                        if full:
                            con.execute(text(
                                f"UPDATE [{schema}].[{table}] "
                                f"SET [{fn_col}]=:fp, ROW_CHANGED_DATE=GETUTCDATE() "
                                f"WHERE [{pk}]=:id"
                            ), {"fp": full, "id": file_id})
                            fixed += 1

                total_fixed += fixed
                st.write(f"**{label}**: {fixed} / {len(rows)} repaired")
            except Exception as e:
                st.error(f"{label}: {e}")

        if total_fixed:
            st.success(f"✅ {total_fixed} path(s) repaired.")
        else:
            st.warning("No matches found — check the directory path.")
...
def _render_seismic(engine):
    ...
    try:
        ...
        st.warning(f"Schema check failed: {_e}")
    st.caption(
        "Catalog SEG-Y (2D and 3D) and P1/90 positioning files. "
        "Optionally seed dbo.SEIS_SET and dbo.SEIS_LINE in PPDM."
    )
    ...
    tab_segy, tab_p190, tab_search, tab_surveys, tab_ppdm, tab_repair = st.tabs([
        "📦 SEG-Y", "📍 P1/90", "🔍 Search", "🗂 Surveys", "🔗 PPDM Mapping", "🔧 Path Repair"
    ])

    # ── Repository selector (shared) ──────────────────────────────────────
    try:
        repos = list_repositories(engine)
        if not repos.empty:
            repo_opts = {"(none — assign later)": ""} | {
    ...
    with tab_segy:
        _render_segy_catalog(engine, repo_opts)

    with tab_p190:
        _render_p190_catalog(engine, repo_opts)
    ...
    with tab_ppdm:
        _render_seis_ppdm_mapping(engine)

    with tab_repair:
        _render_path_repair(engine)
...
def _render_segy_directory(engine, repo_opts):
    _show_catalog_btn(engine, "SEG-Y", "SEIS_FILE_CATALOG", "SEIS_FILE_ID",
                      extra_cols=["SURVEY_NAME","FILE_NAME"], key_suffix="segydir")
    st.divider()
...
def _render_p190_directory(engine, repo_opts):
    _show_catalog_btn(engine, "P190", "SEIS_FILE_CATALOG", "SEIS_FILE_ID",
                      extra_cols=["SURVEY_NAME","FILE_NAME"], key_suffix="p190dir")
    st.divider()
